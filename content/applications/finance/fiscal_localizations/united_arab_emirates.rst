...
.. tip::
   - To create a :guilabel:`Tax Invoice` for a customer, fill in the :guilabel:`TRN` field on the
     customer contact form. If this field is left empty, a :guilabel:`Simplified Tax Invoice` is
     generated instead.
   - If the customer's :abbr:`TRN (Tax Registration Number)` is not available, and a :guilabel:`Tax
     Invoice` is required, enter `N/A` in the :guilabel:`TRN` field on the contact form.
