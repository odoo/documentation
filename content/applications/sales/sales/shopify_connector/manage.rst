...
example, by importing the catalog or using another preferred method), and ensure that categories
and sales taxes are properly configured for accurate results. Refer to the following instructions
for adding products to the **Sales** app.
...
When an order sync or a *Fetch Products* action is done, Odoo goes through the following logic to
...
guidelines <essentials/export_import_data/import-data>`. Ensure that each product variant has its
...
.. example::
   ...
   1. Five sales orders were created in January, containing eight products in their sales order
   lines.
   ...
   :guilabel:`Fetch Products`, and Odoo pulls all 20 products created or updated after January 1.
...
set to the end of the day or the next day. Only products created or updated after that timestamp are
fetched. Click the :icon:`fa-refresh` :guilabel:`Fetch Products` button to pull product data from
Shopify to Odoo.
...
the :icon:`fa-refresh` :guilabel:`Fetch Orders` button in the *Actions* section. Users can customize
the date range by selecting dates in the :guilabel:`Created/Updated between` fields.
...
.. example::
   ...
   **Sales** app. In Odoo, she goes to :menuselection:`Sales app --> Configuration --> Shopify:
   Accounts` and selects the desired Shopify account form. She clicks the :icon:`fa-cog`
   ...
   She copies the series of numbers after `orders/` in the sales order URL. In Odoo, she pastes it
   in the :guilabel:`Ecommerce Order Reference` field and clicks :guilabel:`Recover`.
...
.. tip::
   ...
   :guilabel:`Visit` button next to the *E-commerce Order ID* field on the order form. Clicking this
   button redirects to the associated order's page in Shopify.
...
The **Sales** app automatically detects the currency from the Shopify order. :doc:`Fiscal positions
<../../../finance/accounting/taxes/fiscal_positions>` handle tax mapping from products in Odoo.
...
location or click :guilabel:`Search more` and click :guilabel:`New` to create a :ref:`new location
