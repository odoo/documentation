...
During the sync, Odoo first checks whether a return or refund already exists on the :abbr:`SO
(sales order)` using the unique identifiers provided by Shopify. Only returns and refunds that do
not yet exist in Odoo are synced.
...
the :icon:`fa-truck` :guilabel:`Delivery` smart button to view the *Transfer* page which lists all
stock movements for the :abbr:`SO (sales order)`. Click the warehouse receipt entry with a
