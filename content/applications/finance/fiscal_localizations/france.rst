...
.. list-table::
    ...
    * - :guilabel:`France - Accounting`
      ...
      - This module includes the default :ref:`fiscal localization package
        <fiscal_localizations/packages>`. It applies only to companies based in mainland France,
        excluding those in the DOM-TOMs (Guadeloupe, Martinique, Guyane, Réunion, Mayotte).
    * - :guilabel:`France - Accounting Reports`
      - `l10n_fr_reports`
      - This module enables the export of the French VAT report, which can be sent to the DGFiP, an
        OGA, or a professional accountant. It adds a new "EDI VAT" button to the French VAT report
        and a new "EDI exports" menu item.
    * - :guilabel:`France - Payroll with Accounting`
      - `l10n_fr_hr_payroll_account`
      - This module contains the necessary accounting data for the French payroll rules.
    * - :guilabel:`France - Factur-X integration with Chorus Pro`
      - `l10n_fr_facturx_chorus_pro`
      - This module enables filling three optional fields required for using Chorus Pro,
        particularly when invoicing public services.
    * - :guilabel:`France - FEC Export`
      - `l10n_fr_fec`
      - This module generates the FEC file as defined by the `decree of July 29, 2013
        <http://legifrance.gouv.fr/eli/arrete/2013/7/29/BUDE1315492A/jo/texte>`_, which modifies the
        provisions of Article A. 47 A-1 of the French Tax Procedure Book. It ensures compliance with
        the requirement for companies with computerized accounting to submit a file containing all
        accounting entries for the fiscal year to the tax authorities.
    * - :guilabel:`France - FEC Import`
      - `l10n_fr_fec_import`
      - This module allows the import of FEC standard files, which is useful for importing
        accounting history.
    * - :guilabel:`France - Adding Mandatory Invoice Mentions (Decree no. 2022-1299)`
      - `l10n_fr_invoice_addr`
      - This module adds new address fields required to comply with `Decree 2022-1299
        <https://www.legifrance.gouv.fr/jorf/id/JORFTEXT000046383394>`_, which applies to invoices
        issued after July 1, 2024.
    * - :guilabel:`France - VAT Anti-Fraud Certification for Point of Sale (CGI 286 I-3 bis)`
      - `10n_fr_pos_cert`
      - This add-on ensures compliance with the technical requirements of French regulation CGI art.
        286, I. 3° bis, which outlines specific criteria for the inalterability, security, storage
        and archiving of data related to sales to private individuals (B2C).
...
.. important::
   To comply with the local legislation, :doc:`install </applications/general/apps_modules>` the
   :guilabel:`France - Adding Mandatory Invoice Mentions (Decree no. 2022-1299)`
   `l10n_fr_invoice_addr` module.
...
The `Chorus Pro <https://portail.chorus-pro.gouv.fr/aife_csm>`_ portal, created by the AIFE (Agence
pour l'Informatique financière de l'État), is provided to businesses and must be used to submit
electronic invoices to public entities. It enables the submission and management of invoices,
tracking of their processing status, and provides information about the actual payment. Since
January 2020, all suppliers to the public sector (B2G) have been required to issue invoices
electronically. Odoo supports integration with Chorus Pro to submit invoices generated in Odoo.

Ensure the :guilabel:`France - Factur-X integration with Chorus Pro` (`l10n_fr_facturx_chorus_pro`)
module is :doc:`installed </applications/general/apps_modules>`.

.. seealso::
   `How to use Chorus Pro <https://portail.chorus-pro.gouv.fr/aife_documentation>`_
...
To configure Chorus Pro contacts, follow these steps:

#. In the :guilabel:`Sales & Purchase` tab, verify the :guilabel:`Country` and :guilabel:`Tax ID`
...
When a new invoice is created, fill in the following fields in the :guilabel:`Other Info` tab:

- :guilabel:`Buyer Reference`
- :guilabel:`Contract Reference`
- :guilabel:`Purchase Order Reference`

Once the invoice is confirmed, click :guilabel:`Send & Print`. Then, in the :guilabel:`Send` window,
enable :guilabel:`Peppol Bis Billing 3.0` and click :guilabel:`Send & Print`.
...
.. note::
   ...
   these entries may need to be deleted in the User Interface. Odoo automatically marks these RAN
   entries as unnecessary.
...
.. note::
   ...
   header, defining the list of fields for each entry, and each following line representing a single
   accounting entry without any specific order.

The :guilabel:`France - FEC Import` module requires the files to meet the following technical
specifications:
...
.. note::
   It is not an issue if the field is not filled out on the line, but the entry still needs to be
   reconciled with an unrecorded payment. The account is flagged as reconcilable as soon as the
   import of the move lines requires it.
...
The matching is done by comparing the left-most digits, starting with all digits, then 3, then 2.
...
New journals have their name prefixed with :guilabel:`FEC-`. For example, :guilabel:`ACHATS` ->
:guilabel:`FEC-ACHATS`
...
Each partner keeps its :guilabel:`Reference` from the :guilabel:`CompAuxNum` field.

.. note::
   These fields are searchable in accordance with previous FEC imports for fiscal/audit purposes.

.. tip::
   Users can merge partners using the Data Cleaning App, where Vendors and Customers or similar
   partner entries may be merged by the user, with the system assisting by grouping similar entries.
...
If a line includes partner information, it is copied to the accounting move itself provided the
...
:doc:`Install </applications/general/apps_modules>` the :guilabel:`France - FEC Export`
(`l10n_fr_fec`) module to download the FEC. To do so, go to :menuselection:`Accounting --> Reporting
--> FEC`. In the :guilabel:`FEC` window, fill in the following fields:
...
Only users with *Manager* access rights can initiate the inalterability check:
...
The system also processes automatic sales closings on a daily, monthly, and annual basis. Such
closings compute the sales total for the period and the cumulative grand totals from the very first
...
.. note::
   ...
   - POS orders are posted as journal entries at the closing of the POS session. Closing a POS
     session can be done anytime. To prompt users to do it on a daily basis, the module prevents
     them from resuming a session that was opened more than 24 hours ago. Such a session must be
     closed before selling again.
...
Users are responsible for their Odoo system and must operate it carefully. Modifying the source code
that ensures data integrity is prohibited.
