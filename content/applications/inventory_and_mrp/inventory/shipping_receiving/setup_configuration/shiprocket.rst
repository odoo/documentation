...
the company's Shiprocket account. Follow the website's steps to complete registration.
...
For details to configure the other fields on the shipping method, such as :guilabel:`Delivery
...
couriers to use in the :guilabel:`Shiprocket Couriers` field. If this field is left blank, Odoo
selects the first courier based on the :ref:`courier priority <inventory/shiprocket/enable-couriers>`
...
#. **Warehouse address**: By default all packages are sent from the specified address in the
   warehouse. Make sure to set the :guilabel:`Address` in the warehouse form.
...
.. note::
   Shipping costs for Shiprocket can be automatically generated in both the **Sales** and
...
If a delivery order is cancelled in Odoo, it is automatically archived in Shiprocket. However,
cancellation notices are not sent to the courier itself, so make sure to log in to the courier's
