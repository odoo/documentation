...
UrbanPiper is an order management system that integrates with multiple food delivery platforms. It
consolidates orders from all connected platforms into a single interface, simplifying the delivery
process.
...
.. tip::
   Create your Atlas account directly from the :ref:`POS settings <configuration/settings>` by
   clicking :guilabel:`Fill this form to get Username & Api key` under the :guilabel:`Food Delivery
   Connector` section.
...
#. Go to the :ref:`POS settings <configuration/settings>`, scroll down to the :guilabel:`Food
   Delivery Connector` section, and enable the :guilabel:`Urban Piper` setting.
...
#. Click the :guilabel:`Create Store` button. Doing so creates a new location on the UrbanPiper
   ...
   .. note::
      ...
      - When creating a store, the locations' update on the UrbanPiper Atlas platform can take 2-3
        minutes.
      - The store is automatically named after your point of sale name.
   .. image:: urban_piper/create-store.png
      :alt: UrbanPiper settings
...
#. Go to the :ref:`POS settings <configuration/settings>` and scroll down the :guilabel:`Food
   Delivery Connector` section.
...
#. In the popup window:
   ...
   #. Enter the :guilabel:`Platform ID` and :guilabel:`Platform URL` in the corresponding
      fields to establish the connection between the platform and UrbanPiper.
...
Sometimes, the shop or restaurant may want to **reject** an order. In this case, open the
orders' list view,
...
#. Select one of the reasons from the popup window.
   ...
   .. important::
      **Swiggy** orders cannot be directly rejected. Attempting to reject one prompts Swiggy
      customer support to contact the restaurant. Similarly, **Deliveroo**, **JustEat**, and
