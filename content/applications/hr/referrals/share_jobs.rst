...
button on the main **Referrals** app dashboard. This presents all job positions, with each
...
To share an individual job position with someone via email, click the :icon:`fa-envelope-o`
:guilabel:`Send Email` button on the specific job position card. A preconfigured :guilabel:`Send Job
Offer by Mail` pop-up window appears. Enter the recipient's email address in the :guilabel:`Email`
field.
...
The :guilabel:`Body` is prepopulated with: `Hello! There is an amazing job offer for (Job Position)
in my company! It will be a fit for you (link to Job Position)`.

Modify the message, if desired, then click the :guilabel:`Send SMS` button to send the message, or
...
The :guilabel:`Body` is prepopulated with: `Hi! (Company name) is hiring for a (Job position)
position. I think you would be a great fit for this role. If you are interested, please apply here:
(link to Job Position).`

Modify the message, if desired, then click the :guilabel:`Send WhatsApp` button to send the message,
or click :guilabel:`Cancel` to close the pop-up window and cancel the message.
...
preconfigured with :guilabel:`Job for you`, but can be edited.

The email :guilabel:`Body` is also populated with preconfigured text. The text that appears is:

`Hello,`

`There are some amazing job offers in my company! Have a look, they can be interesting for you\:
(link to all active job positions)`
