...
Services can be logged in advance, such as regular maintenance, like oil changes or tire
rotations, or after unexpected repairs are done.
...
- :guilabel:`Service Type`: Using the drop-down menu, select the type of service performed. Or, enter
...
- :guilabel:`Date`: Using the calendar selector, select the date the service was provided, or
  is scheduled to be performed. Navigate to the desired month using the :icon:`oi-chevron-left`
...
- :guilabel:`Vendor`: Using the drop-down menu, select the vendor who is performing the service. If the
  vendor has not already been entered in the system, :ref:`add and configure the vendor
  <fleet/new-vendor>`.
...
- :guilabel:`Driver`: The vehicle's current driver populates this field when the :guilabel:`Vehicle`
  is selected. If the driver needs to be changed, another driver can be selected using the drop-down menu.
- :guilabel:`Odometer Value`: Enter the odometer reading from when the service was done. The units of
  ...
  .. tip::
     ...
     :guilabel:`(Internal Link)` icon to the right of the vehicle selected in the :guilabel:`Vehicle` field.
...
- :guilabel:`NOTES`: Enter any notes for the repair at the bottom of the service form. For example, this
  can include estimate details or parts being replaced.
...
Service types should be created from a :ref:`service form <fleet/service-form>`. No other way exists
...
When a service is done for the first time, typically, the vendor is not in the database. It is
best practice to add the full details for a vendor in the database, so that any necessary
information can be retrieved.

Vendors are added with the **Contacts** app. Refer to the :doc:`documentation <../../essentials/contacts>`
for more details.
