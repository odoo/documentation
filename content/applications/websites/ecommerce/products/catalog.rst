...
various filters such as :ref:`attribute <ecommerce/catalog/attributes>` filter.
...
.. tip::
   It is also possible to customize the individual
   :ref:`product pages <ecommerce/products/product-form>`.
...
You can also create new ribbons by clicking on the green :icon:`fa-plus` (:guilabel:`Create`) icon
next to the :guilabel:`Ribbon` field. Then add a :guilabel:`Ribbon name` and define its
:guilabel:`Position`: :guilabel:`Left` or :guilabel:`Right`. Choose a :guilabel:`Background` and
a :guilabel:`Text` label. Once saved, you can still :guilabel:`Edit` it by clicking
the :icon:`fa-pencil-square-o` (:guilabel:`Edit`) icon next to the :guilabel:`Ribbon` label.

.. image:: catalog/catalog-ribbons.png
   :alt: Create a new ribbon.

This ribbon is now available for all the eCommerce products.

.. tip::
   - There are other ways to create a new ribbon:

     - Go to :menuselection:`Website --> eCommerce --> Product Ribbons` and click :guilabel:`New`.
       On the product ribbon form, add a :guilabel:`Ribbon name` and define its :guilabel:`Position`
       (:guilabel:`Left` or :guilabel:`Right`). You can also set a :guilabel:`Text Color` and a
       :guilabel:`Background Color`.
     - Activate the :doc:`developer mode <../../../general/developer_mode>`
