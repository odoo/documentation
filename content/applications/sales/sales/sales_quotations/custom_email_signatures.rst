...
.. example::
   In this use-case, the `Sales: Order Confirmation` email template is used to illustrate this
   concept.
...
On the email template, click into the *Email Configuration* tab. Delete the default text in the
:guilabel:`From` field and replace it with: `{{ (user.email_formatted) }}`.

.. image:: custom_email_signatures/from-field.png
   :alt: The from field updated with the correct text.

Modify the code
===============

Click into the *Content* tab, and highlight some text so the toolbar is visible. Click the
:guilabel:`</> (Code view)` button and the text changes to code.

Search the text for `signature` to find the two instances of the code `object.user_id.signature`.
Replace both `object.user_id.signature` instances with `User.signature`.

.. image:: custom_email_signatures/code.png
