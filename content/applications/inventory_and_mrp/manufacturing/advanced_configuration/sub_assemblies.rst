...
Replenishment is critical for multilevel BoMs to prevent bottlenecks, manage lead times, and
optimize inventory. Without it, missing components can halt production, delay orders, and increase
costs. A well-planned replenishment strategy ensures just-in-time availability, automates
procurement, balances stock levels, and keeps supply chains efficient. This minimizes delays,
reduces manual effort, and ensures smooth manufacturing.
...
create the top-level product by navigating to :menuselection:`Manufacturing --> Products -->
...
.. note::
   Learn more about :doc:`reordering rules <../../purchase/products/reordering>` and about
   :doc:`replenishing stock on order <../../inventory/warehouses_storage/replenishment/mto>`
...
In this flow, a multilevel |BOM| gets created, initial inventory is set, a 0/0/1 reordering rule is
created (the recommended production plan), lead times get set, and a production options get
configured.
...
Follow the steps in the :ref:`Create a multilevel BoM <Create a Multilevel BoM>` section to build
the |BOMs|.
...
Vendor and manufacturing lead times will lead to better predictions for production and order
fulfillment. Set vendor lead times for components that are purchased. These can appear at any level
in a multilevel |BOM| except the final product. Set manufacturing lead times for products that are
built using a |BOM|. These can appear at any level in a multilevel |BOM| except the lowest level
(when individual components are procured).

.. tip::
   Learn more about :doc:`vendor lead times and manufacturing lead times
   <../../inventory/warehouses_storage/replenishment/lead_times>`, like how to set a lead time.
...
- **Master Production Schedules**: If manually planned manufacturing orders are needed (e.g., to
...
.. tip::
   Continue configuring the database by setting up :doc:`sales quotations
   <../../../sales/sales/send_quotations/create_quotations>` and :doc:`eCommerce shop pages
   <../../../websites/ecommerce/products/catalog>`
