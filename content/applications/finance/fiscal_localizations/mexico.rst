...
.. list-table::
   ...
   * - :guilabel:`Mexico - Payroll`
     - `l10n_mx_hr_payroll`
     - Adds the fields necessary to manage employee data for :doc:`Payroll <../../hr/payroll>` and
       :doc:`Time Off <../../hr/time_off>`
...
In the XML Odoo will try to distribute the discount between the lines evenly, while any math
...
.. warning::
   This section might require technical knowledge may risk adding technical debt; it is recommended
...
the Mexican localization requires each payment method needs to be configured with a correct
...
Cost` field can be edited at any time to fix any mistakes that could occur during the landed cost,
this changes the :guilabel:`Customs number` and the name.
...
vehicle setup is managed directly on the vehicle, all the required fields are only visible after
