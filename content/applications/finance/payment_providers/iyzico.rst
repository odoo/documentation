...
#. Navigate to :menuselection:`Settings --> Merchant Settings`, copy the values of the
   :guilabel:`API Key` and :guilabel:`Secret Key` fields, and save them for later.
#. On the same :guilabel:`Merchant Settings` page, search for and enable :guilabel:`Merchant
   Notifications`.
#. | Enter your Odoo database URL followed by `/payment/iyzico/webhook` in the :guilabel:`Merchant
