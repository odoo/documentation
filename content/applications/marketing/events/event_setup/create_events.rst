...
- :guilabel:`Language`: The chosen language for all event communications.
   ...
   .. note::
      To the right of the entered :guilabel:`Event Name`, there is a language tooltip, represented
      by an abbreviated language indicator (e.g., `EN`). When clicked, a *Translate: name* pop-up
      window appears, displaying various preconfigured language translation options available in the
      database.
...
- :guilabel:`Website`: The specific :doc:`website
  ...
  database.
- :guilabel:`Published`: The toggle button to publish or un-publish the website for attendees.
...
.. _events/ticket-combos:

Set up ticket combos
~~~~~~~~~~~~~~~~~~~~

Optionally, users can combine the event registration ticket with separate products (e.g., food,
beverages, or merchandise) to create a combo.
...
Attendees who register for the specific ticket :ref:`from the website <events/sell-tickets/website>`
see the additional products included with their ticket on the *Order summary* page before confirming
their order.

.. example::
   A company is hosting an event with three different ticket levels:

   - *Basic*: Only includes admission.
   - *Silver*: Includes admission and a casual T-shirt.
   - *Gold*: Includes admission, a T-shirt, and pants.

   In the *Tickets* tab, three event registration products are added along with their names and the
   relevant :guilabel:`Additional Products`.

   .. image:: create_events/ticket-combo-example.png
      :alt: Example of a ticket combo on an event form.
...
:guilabel:`Create New` to create a new one. Doing so opens a new *Create Question* pop-up window.
...
To publish an event, toggle the :guilabel:`Published` button on the event form.

Alternatively, click the :icon:`fa-globe` :guilabel:`Go to Website` smart button to open the event
webpage. Then, toggle the :guilabel:`Unpublished` button in the header menu, changing it to a
