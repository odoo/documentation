...
You will find a toggle labeled **Managed by Odoo**.
If you wish to use the Fiskaly service through Odoo, you need to enable this option.
Enabling it allows Odoo to manage the Fiskaly integration on your behalf.

If you already have a Fiskaly account and prefer to use your own credentials, you can disable this
toggle and configure your existing Fiskaly credentials manually.

  .. image:: austria/management-toggle.png
    :alt: Management toggle

For testing purposes, you should make sure to use the Test Mode. This allows you to test the
integration without affecting the production server. By default, Test Mode is enabled, so don’t forget to
disable it before going live in production. You can toggle this setting in the :guilabel:`Fiskaly` tab on the
company form by switching off the Test Mode option.

.. important::
   Once you authenticate your credentials, you can change neither the **Management Option** nor **Mode of operation**
   of the company.
...
You can find these credentials in your FinanzOnline account. If you don't have a FinanzOnline account, you can create one by visiting the
...
.. note::
   ...
   - For the orders which are unsigned due to unavailability of SCU, you can use the **Sign Orders**
     action to sign the orders manually. The **Sign Orders** action is only available for the orders
     that are not signed yet. You can find them by unhiding the **Signed** field in the list view of
     the orders.
