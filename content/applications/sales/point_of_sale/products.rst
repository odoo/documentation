...
.. tip::
   ...
   then select :icon:`fa-plus` :guilabel:`Create Product`. Enter the product details in the popover
   and click :guilabel:`Save`. The product is immediately available in the POS register. Existing
   products can also be updated directly from the POS register through the :ref:`product information
   popover <pos/products/information-display>`.
...
- Enable the :guilabel:`To Weigh` option if the product must be weighed. Configure a connected
  :doc:`scale <hardware_network/scale>` for this purpose.
...
.. important::
   To enable and use product variants, install one of the following apps, depending on your business
   needs:

   - :doc:`Inventory app </applications/inventory_and_mrp/inventory>`
   - :doc:`Sales app </applications/sales>`
   - :doc:`eCommerce app </applications/websites/ecommerce>`
   - :doc:`Purchase app </applications/inventory_and_mrp/purchase>`
   - :doc:`Rental app </applications/sales/rental>`

Once the required app is installed, open the corresponding app and navigate to
:menuselection:`Configuration --> Settings`. Then enable :guilabel:`Variants` (or :guilabel:`Product
Variants`), and :guilabel:`Save`.
...
the :guilabel:`Attributes & Variants` tab.

.. tip::
   - When configuring an attribute value, use the :guilabel:`UrbanPiper Locations` field to select
     the POS where the attribute value should be synchronized for :doc:`online food delivery
     <restaurant/urban_piper>`.
   - Navigate to :menuselection:`Point of Sale --> Configuration --> Attributes` to manage all
     configured attributes, or to :menuselection:`Point of Sale --> Products --> Product Variants`
     to view all variants.
...
.. example::
    ...
    .. tip::
       ...
       - When all products that belong to a predefined combo are added to the cart individually,
         ...
         If multiple combos are detected, click :guilabel:`Choose`, and :guilabel:`Apply` the
         appropriate combo.
...
set to :guilabel:`Goods` or :guilabel:`Combo`. Enable the :ref:`Track Inventory option
<inventory/product_management/tracking-inventory>` to track products by quantity. Products can also
be tracked :ref:`by lot or by serial number <pos/products/sn>`.
...
#. Install the :doc:`Inventory app </applications/inventory_and_mrp/inventory>` if it has not
   already been installed.
#. :ref:`Enable the Lots & Serial Numbers setting
   <inventory/product_management/traceability-setting>`.
#. :ref:`Configure products <pos/products/creation>`, and select an option from the
   :guilabel:`Tracking` dropdown menu.
