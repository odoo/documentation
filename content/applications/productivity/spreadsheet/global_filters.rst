...
.. |global-filter| replace:: :raw-html:`<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewbox="0 0 20 20"><path fill="currentColor" d="M1 3h12L7 9M5.5 6h3v11l-3-3M14 4h4v2h-4m-3 3h7v2h-7m0 3h7v2h-7"></path></svg>`
.. |drag-handle| replace:: :raw-html:`<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 4 16" fill="currentColor"><circle cx="2" cy="3.5" r="1"></circle><circle cx="2" cy="6.5" r="1"></circle><circle cx="2" cy="9.5" r="1"></circle><circle cx="2" cy="12.5" r="1"></circle></svg>`

Global filters, represented by the |global-filter| :guilabel:`Filters` icon at the top right of an
Odoo spreadsheet, allow you apply one or more filters to all the Odoo data that has been
:doc:`inserted in that spreadsheet <insert>`.
...
.. tip::
   When a spreadsheet with global filters is added to a dashboard, the filters appear as drop-down
...
- :ref:`Date <spreadsheet/global-filters/create-date>`: filters data based on a specific time range,
  e.g., :guilabel:`Month / Quarter`, :guilabel:`Relative Period`, or :guilabel:`From / To`.
- :ref:`Relation <spreadsheet/global-filters/create-relation>`: filters data based on a relational
  field in a related model, e.g.,  the :guilabel:`Salesperson` field in the *Sales Order* model or
  the :guilabel:`Customer` field in the *Contact* model.
...
.. tip::
   - Global filters work by adding extra conditions to the domains of all the data sources in the
     spreadsheet. Therefore, if you intend using global filters, do not use the same conditions when
     configuring the initial list or pivot table.
   - Setting default values, where relevant, ensures that the spreadsheet or dashboard loads quickly
     ...
     :guilabel:`Date` filter could be set to show data from the last 30 days upon opening.
...
To function as intended, a global filter needs to act on the correct database field. Consider
...
Open the desired spreadsheet from the Documents app or via the Dashboards app if you are adding
...
To add a new filter, click |global-filter| :guilabel:`Filters` then, under :guilabel:`Add a new
filter...` click the :guilabel:`Date`, :guilabel:`Relation` or :guilabel:`Text` button  as
appropriate. The :guilabel:`Filter properties` pane opens.
...
#. From the :guilabel:`Time range` drop-down menu, select one of the following:

   - :guilabel:`Month / Quarter`: enables a drop-down menu of specific months and/or quarters and a
   ...
   - :guilabel:`Relative Period`: enables a drop-down menu of specific time ranges relative to the
...
#. In the :guilabel:`Field matching` section, for each data source click next to
   :guilabel:`Date field` and select the field the date filter should match with.

   The :guilabel:`Period offset` defaults to :guilabel:`No offset`.
   ...
   .. tip::
      ...
      range by one or more relative periods in the past or future. The options available are:
      :guilabel:`Previous` and :guilabel:`Next`.

      Using this feature, you could, for example, compare data for Q4 of the chosen year with data
      from the previous period, i.e., Q3 of the same year.

      To compare data effectively, duplicate the relevant data source and apply the filter *without*
      a period offset to the first data source and *with* a period offset to the second data source.

#. Once all the information is entered, click the :guilabel:`Save` button. If any of the fields
   entered in the :guilabel:`Field matching` section are not the correct type of field, an
   error is shown stating :guilabel:`Some required fields are not valid`.
...
#. In the :guilabel:`Related model` field, start typing the model name to reveal a list of all
   ...
   :guilabel:`Default value` and :guilabel:`Possible values` fields and the
   :guilabel:`Field matching` section appear.
...
#. In the :guilabel:`Field matching` section, check if the correct matching field has been
   ...
   .. note::
      A :guilabel:`Relation` filter can only match with a relational field, i.e., a :ref:`Many2One
      <studio/fields/relational-fields/many2one>`, :ref:`One2Many
      <studio/fields/relational-fields/one2many>`, :ref:`Many2Many
      <studio/fields/relational-fields/many2many>` or :ref:`Related
      <studio/fields/relational-fields/related-field>` field.

#. Once all the information is entered, click the :guilabel:`Save` button. If any of the fields
   entered in the :guilabel:`Field matching` section are not from the right model or are not the
   right type of field, an error is shown stating :guilabel:`Some required fields are not valid`.
...
#. In the :guilabel:`Field matching` section, for each data source click below the data source name
   ...
   .. note::
      ...
      :ref:`Decimal <studio/fields/simple-fields/decimal>` field.

#. Once all the information is entered, click the :guilabel:`Save` button. If any of the fields
   entered in the :guilabel:`Field matching` section are not appropriate, an error is shown
...
.. example::
   ...
   product from the :guilabel:`Product` filter and have both the pivot table and graph only show
   ...
   With this configuration, the pivot table and graph can be filtered by product name by
...
Click the |global-filter| :guilabel:`Filters` icon at the top right of an Odoo spreadsheet to
access the global filters that have been created for that spreadsheet.
...
- **Apply one or more global filters** by selecting appropriate values per filter, as relevant.
  ...
  .. tip::
     ...
     value, as relevant. To refresh data in an inserted list, pivot table or graph without losing
     global filters that have been applied, click :menuselection:`Data --> Refresh all data` from
     the menu bar.

- **Change the order** of existing filters by hovering over a filter and using the |drag-handle|
...
- **Delete an exiting filter** by selecting the :icon:`fa-cog` :guilabel:`(Edit)` icon to open the
