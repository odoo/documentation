...
.. note::
   Because dropshipped products are not stored in the company's inventory, these products are *not*
   reflected in the *Locations* report and *Aging Report*.

   This differs from :doc:`consignment <owned_stock>`, which is stored in an internal location (and
   stored in *Location* report and *Moves Analysis*), but is excluded from *Stock Valuation*
   reports.
...
for. Multiple vendors can be added, but the vendor at the top of the list is automatically selected
for purchase orders.
...
:guilabel:`Purchase` smart button at the top of a sales order to view the associated :abbr:`RFQ
...
To view all dropship orders, navigate to the :menuselection:`Inventory` :guilabel:`Overview`
