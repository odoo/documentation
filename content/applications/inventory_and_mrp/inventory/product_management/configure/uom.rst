...
By default, when a customer orders multiple units of the same product at a POS register, the
register and the :doc:`customer display
<../../../../sales/point_of_sale/hardware_network/customer_display>` shows each unit of the product
on a separate line, thus cluttering the display.
...
After this setting is enabled, multiple items of the same unit are grouped on the POS register. The
number of that unit of measure and the total price of the products are displayed.
