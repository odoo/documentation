...
#. When running a blog, enable :guilabel:`Import Blogs` to extract blog post data.
   ...
   .. note::
      Odoo automatically detects the blog platform used by the original website. Blog extraction is
      supported only for websites that use `WordPress <https://wordpress.com/>`_. The platform
      provides a public API that allows Odoo to extract blog data without any additional
      configuration.
