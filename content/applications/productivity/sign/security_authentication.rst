...
- requiring other methods of :ref:`secured identification <sign/security/authentication>`, such as SMS,
  Aadhaar eSign (India) or itsme® (European Union, United Kingdom, Norway and Iceland)
...
Each time a document is fully signed, a certificate of completion is generated and sent to all
signers via email. When a document is signed via a record's chatter, the certificate of completion
is also added to the chatter, along with the fully signed document.
...
- :guilabel:`Document Details`, which include when the document was created and by who, the number
  of signers, and a unique reference hash that can optionally be added to each page of a signed
  document.
