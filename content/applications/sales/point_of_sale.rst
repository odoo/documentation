...
- All POS transactions are recorded in the :doc:`Invoicing or, if installed, Accounting
...
select the card that best aligns with the business. This action automatically generates a POS
preconfigured for the selected business type.

To create additional POS or to create a POS from scratch, go to :menuselection:`Point of Sale -->
...
.. important::
   Ensure each POS has a dedicated cash payment method, associated with a dedicated cash journal.

.. seealso::
   :doc:`point_of_sale/payment_methods`
...
- To **customize** the **header** and **footer**, activate :guilabel:`Header & Footer` setting and
...
- To **print receipts** automatically once the payment is registered, enable the
  :guilabel:`Automatic Receipt Printing` setting.
- To print receipts that don't display the product prices, enable the :guilabel:`Basic Receipt`
  setting.
...
Customers' contact details, such as their phone number or email addresses, can be stored
automatically when sending receipts by email, SMS or Whatsapp to use them later. Configuring the POS
to store customer's information depends on the type of information to store:

- **Email addresses**: they are automatically collected and saved in POS orders when sending a
  receipt by email.
- **Phone numbers**: they are automatically collected once receipts are set to be sent by SMS or
  Whatsapp. To activate this feature, follow these steps:
...
.. seealso::
   Store contact details can be used for :doc:`marketing <../marketing>` purposes.
