...
has a *Training Attendances* report that tracks completed employee training. Managers can use this
report to identify employees who have completed the required training for a specific role or
project.
...
:menuselection:`Employees app --> Learning --> Training Attendances`. Click the :guilabel:`New`
button, and a blank line appears at the bottom of the report. Configure the following fields on the
line:
...
- :guilabel:`Date`: Using the calendar selector, select the date the training was completed. This
  field is populated with the current date by default.
...
As the information is entered, Odoo automatically saves the entry. Refresh the page to reload the
report. The entry is moved to the correct place on the report, based on the date.
