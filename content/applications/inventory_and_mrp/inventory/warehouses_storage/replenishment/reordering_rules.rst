...
Example where hozion days feature is triggered
----------------------------------------------

A product shipped from Asia has a combined vendor lead time of 30 days and a shipping cost of $100,
including :doc:`landed costs <../../inventory_valuation/landed_costs>` and tariffs.
...
Setting :guilabel:`Horizon Days` to `20.0` Odoo extends the forecasted date by 20 days:

This allows Odoo to “look ahead” through December 24, detecting that |SO| 2's demand (December 19)
falls within that horizon.

As a result, Odoo groups |SO| 1 and |SO| 2 in a single replenishment, reducing shipping costs by
consolidating orders.

|SO| 3, which is due on December 25, falls just outside the horizon window and is not included in
the same replenishment.

Example where horizon days feature is not triggered
---------------------------------------------------

Considering the example above, if |SO| 1 does not exist, then:

- **November 4**: Current date. The forecasted date is December 4 (30 lead time).
- **November 5**: The forecasted date shifts to December 5.
- |SO| 2: Requires the product by December 19. Odoo will only trigger the order on November 19, when
  the extended forecasted date (December 5 + 20 days = December 25) covers |SO| 2's demand.

This shows that Horizon Days help plan replenishment in advance by extending the forecasted horizon,
allowing users to anticipate future needs while maintaining just-in-time efficiency.
