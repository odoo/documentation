...
.. note::
   - When a line in a request for quotation or purchase order includes an analytic distribution, a
     :guilabel:`Budget` smart button appears, providing a link to the budget report for more
     details.
...
should have been received based on the current date relative to the start/end dates. Clicking
:guilabel:`Details` opens a filtered view of the :ref:`budget report
<accounting/reporting/budgets/analytic-budget-reporting>` related to that specific budget line.
...
yearly) for the selected :guilabel:`Analytic Plans`. Odoo generates a new budget for each
:guilabel:`Period` between the start and end dates:
