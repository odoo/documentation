...
Spreadsheets in `.osheet.json` format, i.e., that have been downloaded from another Odoo database to
be :ref:`transferred <spreadsheet/manage-spreadsheets/transfer>` to the current database, can be
uploaded in a similar way and opened directly. In this case, the file remains an `.osheet.json`
file, but can be edited fully edited in Odoo Spreadsheet.
...
- :icon:`os-copy-file` :guilabel:`Make a copy`: creates a duplicate of the current spreadsheet with
  the same :ref:`settings <spreadsheet/manage-spreadsheets/regional-settings>`.
...
With :ref:`developer mode <developer-mode>` activated, clicking :icon:`os-download-as-json`
:menuselection:`Download as JSON` downloads the spreadsheet in `.osheet.json` format. This format is
needed if you are :ref:`transferring a spreadsheet <spreadsheet/manage-spreadsheets/transfer>`
containing dynamic Odoo data between two databases.
...
- Restore an earlier version by clicking :icon:`fa-ellipsis-v` :guilabel:`(vertical ellipsis)` then
  :guilabel:`Restore this version`.
...
- Create named versions by clicking on the date and time of the relevant version and entering the
  desired name. The date and time of the version are then displayed below the new name.
...
.. _spreadsheet/manage-spreadsheets/transfer:

Transfer a spreadsheet between databases
----------------------------------------

When setting up standard reporting across multiple databases, or for training purposes, it may be
useful to transfer a :doc:`spreadsheet containing dynamic Odoo data <spreadsheet/insert>` from one
database to another. To do so:

#. In the relevant spreadsheet in the source database, with :ref:`developer mode <developer-mode>`
   activated, click :menuselection:`File -->` :icon:`os-download-as-json` :menuselection:`Download
   as JSON` from the menu bar and save the `.osheet.json` file on your device.
#. In the target database, from the main Odoo Documents screen, click :menuselection:`New -->
   Upload`, then select the downloaded `.osheet.json` file and click :guilabel:`Open`.
#. Click on the uploaded file to open it in Odoo Spreadsheet.

The uploaded file remains an `.osheet.json` file, but acts like an Odoo spreadsheet and retains its
functionality provided all models and fields referenced in the spreadsheet are identically named
in the target database.

.. tip::
   Spreadsheet templates containing dynamic Odoo data can be transferred between databases in a
   similar way. :ref:`Download the template <spreadsheet/templates/manage>` in `.osheet.json`
   format, then, in the target database, upload the file, open it, and click :menuselection:`File
   -->` :icon:`os-save` :menuselection:`Save as template` from the menu bar.
...
By default, a new spreadsheet inherits the locale of the owner's user profile. For example, if the
language of your user profile is set to :guilabel:`French (BE) / Français (BE)`, any spreadsheets
you create will follow Belgian French conventions.

.. tip::
   When you open a spreadsheet that has a different locale than your own, a blue :icon:`fa-globe`
   :guilabel:`(globe)` icon appears at the top right of the spreadsheet. Hovering over the icon
   reveals a warning message that indicates the spreadsheet locale and highlights formats that
   differ.
