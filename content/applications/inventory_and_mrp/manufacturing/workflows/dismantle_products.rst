...
Set up a :ref:`new BoM <manufacturing/basic_setup/bom-setup>`. Be sure to name the
product in a way that differentiates it from the product being disassembled.
...
:ref:`Add by-products <manufacturing/bill_configuration/by-products>` to the |BoM| that define the
components that will be entered into inventory as a result of an |MO|.
...
Enter the :guilabel:`Lot/Serial Number` for each by-product, then click :guilabel:`Save`.
