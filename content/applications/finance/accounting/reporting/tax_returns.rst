...
.. seealso::
   - `"What is VAT?" <https://ec.europa.eu/taxation_customs/business/vat/what-is-vat_en>`_.
   - :doc:`../taxes`
   - :doc:`../get_started`
...
.. note::
   Taxes and reports are usually pre-configured in Odoo: A :ref:`fiscal localization package
   <fiscal_localizations/packages>` is installed according to the country selected during database
   creation.
...
In the Accounting dashboard, complete the following actions displayed in the :guilabel:`Tax Returns`
journal:
...
Any new transaction whose accounting date is before the :guilabel:`Lock Tax Return` date has its tax
values moved to the next open tax period. This prevents any changes to a report after its period is
closed.

Locking the tax date before processing the :guilabel:`Closing Journal Entry` prevents other users
from modifying or adding transactions that would impact the final report, reducing the risk of tax
declaration errors.
...
In the Accounting Dashboard, the :guilabel:`Tax Returns` journal displays automatic reminders to
...
- An action steps status view that displays applicable steps such as :guilabel:`Review`,
  :guilabel:`Submit`, and :guilabel:`Pay`.
- Action buttons: Odoo highlights them in purple to indicate their priority.
...
.. note::
   ...
   - When the :ref:`Review <accounting/tax-returns/vat-return-review>`, :ref:`Submit
     ...
     in green in the action steps status view.
...
To open a tax return, click the local VAT report smart button (or :guilabel:`Generic Tax
report`) on the corresponding tax return line in the :guilabel:`Tax Return` view.
...
.. Tip::
   ...
   - Click the :icon:`fa-cog` (:guilabel:`gear`) icon and :guilabel:`Copy to Documents` to save the
     report to the Documents app. Select the format to :guilabel:`Export to`, the
     :guilabel:`Documents Name`, the :guilabel:`Folder` to store it in, and add any
     :guilabel:`Tags`. Then click :guilabel:`Export`.
...
To start the review of a tax return, follow these steps:

#. Click :guilabel:`Review` on the relevant tax return line in the :guilabel:`Tax Return` view to
   display the automatic tax validation checks:

   - :guilabel:`Bank Matching`: Not required for VAT report, but it helps identify any missing
     bills.
...
#. In the :guilabel:`Checks` view, each check card is either marked with a :icon:`fa-check`
   :guilabel:`(pass)` or :icon:`fa-times` :guilabel:`(fail)`. If a check fails, there are two
   options:
...
.. tip::
   ...
   - Once a tax report has been reviewed but not yet submitted, additional invoices or bills from
     ...
     the Tax return dashboard and select :guilabel:`Reset` to review checks before submitting the
     tax return.
...
Once a tax return has completed the :guilabel:`Review` step, proceed as follows:

#. In the :guilabel:`Tax Return` view, click the local VAT report (or :guilabel:`Generic Tax
   report`) from the relevant tax return line to open and review the tax return.
#. After reviewing the tax return, click :guilabel:`Submit`.
...
On submitting the tax return, the :guilabel:`Lock Tax Return` date is automatically updated, and the
closing journal entry is posted in the :guilabel:`Tax Returns` journal.
...
.. note::
   If the :guilabel:`Lock Tax Return` date is not locked before submitting the tax return, Odoo will
   automatically lock the fiscal period on the same date as the accounting date of the closing
...
.. important::
   After the tax report for a period has been posted, Odoo locks that period and prevents new
   journal entries related to VAT. Corrections to customer invoices or vendor bills must be recorded
...
Once a tax return is submitted, a :guilabel:`Payment` window appears if a tax payment is
required. It displays all necessary payment details to complete the transaction, including a QR code
for the banking app (based on the :doc:`fiscal localizations <../../fiscal_localizations>`). There
are two options:
...
To mark a tax payment as completed, click the :icon:`fa-ellipsis-v` :guilabel:`(vertical ellipsis)`
icon and select :guilabel:`Mark as Completed`.

For each pending advance tax payment, click :guilabel:`Pay`. In the :guilabel:`Advance Payment`
window, follow the local recommendations, fill in the required local fields, and use the provided
payment details to finalize the transaction. Two options are available:
