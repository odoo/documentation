...
.. note::
    ...
    in Hoot (the full list can be found in the `@odoo/hoot <https://github.com/odoo/odoo/blob/18.0/addons/web/static/lib/hoot/hoot.js>`_
...
Setting up the test runner is quite straightforward: you just need to have all
the lib files loaded in the assets bundle along with Owl, and then call the main
`start` method entrypoint once all tests and suites have been registered. The tests
will then be run sequentially and the results will be displayed in the **console**
and in the **GUI** (if not running in `headless` mode).
...
it is supposed to be. To do so, it provides a few **modifiers** and a wide range
of **matchers**.
...
others will *transform* that value before performing the assertion on it (i.e. **DOM matchers**).
