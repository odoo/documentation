...
On the journal, add a :ref:`Bank Account Number
<accounting/batch/create-bank-account>` and :ref:`Bank <accounting/batch/create-bank>` if necessary
(such as in the case of creating batch payment files).
...
Odoo's rate. To do so, click :guilabel:`Reset to Odoo's Rate`.
