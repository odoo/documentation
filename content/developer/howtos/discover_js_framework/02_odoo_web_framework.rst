...
.. exercise::
   ...
   - Update the `AwesomeDashboard` component located in :file:`awesome_tshirt/static/src/` to use
     the `Layout` component. You can use `{ "top-right": false, "bottom-right": false }` for the
...
.. example::
   ...
   This example shows how to open the settings view from a component using the action service.
...
Let's improve the dashboard by adding a few cards (see the `Card` component made in the Owl training
) containing a few statistics. There is a route `/awesome_tshirt/statistics` that will perform some
computations and return an object containing some useful informations.
...
- `settings`, optional, for some advance control on the request (make it silent, or using a specific
  xhr instance)
...
probably would prefer to do it only the first time, so we actually need to maintain some state
...
assets bundle, or lazy load it (usually better, since our users will not have to load the chartjs
...
.. exercise::
   ...
   #. In a `Card` (from previous exercises), display a `pie chart
      ...
      correct quantity for each sold tshirts in each size (that information is available in the
      statistics route)
...
.. exercise::
   ...
   #. Make sure your application can be :ref:`translated <reference/translations>` (with `env._t`\ )
