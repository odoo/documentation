...
While Odoo normally reserves existing stock to outgoing demands automatically, users may sometimes
need to manually prioritize specific outgoing orders over other competing demands, especially when
...
allocated, grouped by the |SOs|/|MOs| on which they appear. A link each |SO|/|MO| is included, along
with its :guilabel:`Expected Delivery`, and an option to assign all units to it. Under each
