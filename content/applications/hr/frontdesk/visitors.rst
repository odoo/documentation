...
the :icon:`oi-settings-adjust` :guilabel:`(adjust settings)` icon in the top-right of the list.
...
**only**. Click this button to download a PDF file of the visitor's bade. The badge displays the
...
:icon:`oi-settings-adjust` :guilabel:`(adjust settings)` icon, located at the end of the column name
list. Doing so, reveals a reveals a drop-down menu of column options to enable or disable. A
