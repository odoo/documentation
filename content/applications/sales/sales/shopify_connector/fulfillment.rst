...
:guilabel:`Ready` state and lists the associated sales order's products and their quantities as
separate line items.
...
:guilabel:`Done` state. Similarly, if a pulled order already has the *Fulfilled* tag, Odoo creates
its warehouse delivery form directly in the :guilabel:`Done` state.
...
invoices are automatically created when importing orders from Shopify.
...
the stock movements for the :abbr:`SO (sales order)`. Click the warehouse receipt entry, which has
...
Date`, and the :guilabel:`E-commerce Return Identifier` field displays the unique return identifier
...
Click :guilabel:`Validate` when the return product is received. The status on the warehouse receipt
...
credit note form, the refund date from Shopify is set as the :guilabel:`Invoice Date`, and the
