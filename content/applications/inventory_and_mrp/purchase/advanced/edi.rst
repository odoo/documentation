...
:abbr:`RFQ (request for quotation)`. Doing so converts it into a :ref:`purchase order
<purchase/manage_deals/create-po>`.

.. figure:: edi/po-database-view.png
   :alt: Example PO. The Vendor is the seller's portal user account, Joel.

   Example PO from the buyer's database. The :guilabel:`Vendor` is the seller's portal user account,
   Joel.
...
.. figure:: edi/po-portal-view.png
   :alt: Portal view of the PO, with "Connect your software!" button.

   Joel's portal view of the PO, with the "Connect your software!" button.

.. figure:: edi/pop-up.png
   :alt: Pop-up to copy link.

   Click :guilabel:`Copy` to download.
