...
.. |DIMONA| replace:: :abbr:`DIMONA (Déclaration Immédiate - Onmiddellijke Aangifte)`
...
|DIMONA| is a legal obligation in Belgium for employers.

- It is an electronic declaration that the employer must submit before an employee starts working.
- It is used to officially register the hiring or the end of an employee's contract with the
  authorities (National Social Security Office).
- It allows authorities to monitor employment in real time and ensure everything is compliant
  (social security, declared work, etc.).
- Without a |DIMONA| declaration, the employee is considered undeclared, which can lead to
  penalties.

In short, |DIMONA| is a key tool to combat undeclared work and ensure proper social security
management.
...
Any declaration modifications can also be made through any of these five channels. The channel
...
In Odoo, when the :doc:`Belgian Payroll Localization
<../../hr/payroll/payroll_localizations/belgium>` is installed, Dimona's actions are automatically
available. The five main actions that can be perfoormed are:

#. Check the dimona
#. Open/Send the dimona
#. Update the dimona
#. Close the dimona
#. Cancel the Dimona

.. tip::
   The names of the actions might differ depending of the version you are working with.

.. important::

   To see those actions, some configurations on the Employee's form is required.
...
You need a Belgian Government Compliant Digital Certificate, delivered by Global Sign. This
certificate is essential to send DIMONA's and DMFA. It can be bought `here
<https://shop.globalsign.com/en/belgian-government-services>`_.

.. note::
   If you already have this certificate and created your technical channel, you can skip this
   section.

Below are all the steps to get the certificate.

#. On the home page, click :guilabel:`Buy Now - Belgium`.

   .. image:: belgium/buy-now.png
      :alt: The BUy Now - Belgium button on the main home page.

#. On the *Product Selection* page, click the radio button next to :guilabel:`PersonalSign 3 Pro`,
   then click :guilabel:`Next`.
#. On the *Certificate Application* page, click the radio button next to the number of years being
   purchased, either :guilabel:`1 Year`, :guilabel:`2 Year`, or :guilabel:`3 Year`. Then click the
   radio button next to :guilabel:`Download as .pfx (PKCS12)`, then click :guilabel:`Next`.

   .. image:: belgium/cert-app.png
      :alt: The year selection and download option.

   .. important::
      The option :guilabel:`Download as .pfx (PKCS12)` *must* be selected as this certificate will
      be uploaded in Odoo, which accepts only `.pfx` file formats.

#. In the *Certificate Identity Details* section, enter the name of the person and organization
   using the certificate in the corresponding fields. Ensure the :guilabel:`Country` field has
   :guilabel:`Begium - BE` selected. FInally, enter an email address, create and verify a strong
   password, then click :guilabel:`Next`.

   .. tip::
      As with every password, it's very important to remember it since it is needed in future steps.

      The password must contain only alphanumeric characters, no special characters such as `;”/@#`.

At this stage, a PDF has been generated and has been sent to your email address.
In this email, you will also receive your #order.


In the mail you have received, you have to print the following document "Demande de signature numérique PersonalSign3 Pro"


Once you have printed and filled in the document you should :
Make an appointment with the Chamber of Commerce near you and bring the signed document, a front-and-back copy of your ID card, and €121.
The representative at the Chamber of Commerce will need to sign your contract and then send it back to GlobalSign.
GlobalSign will contact you to verify your identity, this step can take time, and you may need to follow up with them.
Once the authentication is successful, GlobalSign will send an email with the certificate in .pfx format and 2 PDFs that you must keep.


Activate technical Channel
Go on https://www.socialsecurity.be/site_fr/employer/applics/chaman/index.htm and click on "Gérer vos canaux techniques" (= Handle your technical Channels)

You have to create a Webservice < Rest

1/ You have to check those 2 options in order to be able to perform all actions of Dimona in Odoo :
Consultation de la déclaration Dimona
Effectuer des déclarations Dimona

2/ We have to upload the certificate we received from GlobalSign in CRT format in the field "Certificat" in the image above.

How to  change the format ?

PFX -> CRT
openssl pkcs12 -legacy -in my_cert.pfx -out my_cert.crt -nokeys -clcerts
CRT -> CER:
openssl x509 -inform pem -in my_cert.crt -outform der -out my_cert.cer

If you have some difficulties to change the format of your .pfx file, please ask someone who is working in the IT department of your client's company (in Odoo , they know how to do it).
3/ This is the recap of your Technical Channel



At this stage, you have reached the 2 first steps :
Buying a GlobalSign Certificate
Creating a technical Channel ⮕ Webservice<Rest
Now let's go in the Odoo Database and configure it properly.
...
ONSS Company ID = VAT Number of the Company = N°BCE
It's very important that you only write the numbers like this : 1256483541 and NOT 125.648.3541
Registration Number = 9 digits code received from the NSSO (ONSS - RSZ)
It's very important that you only write the numbers like this : 12548245  and NOT 125482-45
DMFA Employer Class = 3 digits code received from the NSSO (ONSS - RSZ)
This information can be also received from the NSSO (ONSS - RSZ)
ONSS Technical User Name
That field should not be filled in for Dimona. You need to fill it in when you want to send the DMFA.
ONSS Technical User Private Key
That field should not be filled in for Dimona. You need to fill it in when you want to send the DMFA.
Expeditor Number ONSS = Numéro d'expéditeur ONSS

You need to fill in the information are under the "Client ID" =⮕ self_service_expeditor_.....
ONSS Certificate
You need to upload the .pfx file that you have bought from Globalsign (see point above).
In order to activate it you need to fill in the password.
Be careful because you need to activate the certificate within a month after you have purchased it.


Employee Form​

All those fields should be filled in in order to open a Dimona
Personal Tab
Name & Surname
Birthday
Birth place
...
Nationality
Gender
Complete address : street, number,zip,city and country
NISS Number

Payroll Tab
Start Date of the contract
Pay Category
Joint Committee (as from 19.1)
Dimona Category (as from 19.2)

Before 19.2, when opening a Dimona declaration for students you need to :
select the correct Pay Category "Belgian Student" on the employee form
specify the number of hours you plan for them on the employee form
As from 19.2 you will be need to specify the DIMONA Category [STU] Student

Here you go, you can now "Open/Send"  Dimona for all your employees!
Congrats!
3. Where can I find the information of Dimona on an employee?
1/ Activate the debug mode
2/ Go in the Employee Application
3/ Open the Payroll tab
4/ All information will be displayed under the section "DIMONA"
...
The relationship between the "employer" and the "worker" couple. This relationship includes stable data (employer identification number, worker's national identification social security number (NISS), start date of the relationship, and optionally end date), which will be maintained even in case of creation, closure, modification, or cancellation of a period.
This relationship begins with the worker's first engagement and ends at the end of their last occupation. In the personnel file, "employer-worker" relationships constitute the first level of consultation. The second level encompasses all Dimona periods;
Dimona Period
...
Dimona Number
A unique number assigned by the NSSO to each Dimona declaration (IN, OUT, UPDATE, CANCEL,...).
For an IN declaration, it is the period identification number. When declarations are submitted via file transfer, multiple Dimona numbers are created: one number per employment relationship included in the structured message;
Dimona Characteristics
Mandatory data of a Dimona declaration.
There are four: the industry sector number, worker type, sub-entity (reserved for certain public sector employers), and user (for temporary workers). Characteristics are always linked to a period. In case of different characteristics, multiple periods can coexist;
Receipt Confirmation
Confirms for web users that the declaration has been received (appears immediately on the screen);
Receipt Acknowledgment
Electronic message for batch senders indicating whether the file is usable for the NSSO.
This "receipt acknowledgment" contains the ticket number (= file identification number) and the file status: accepted or rejected depending on whether the file is readable or not;
Notification
...
It can be of three types: positive (Dimona accepted), negative (Dimona rejected), or provisional (only in case of problems with worker identification).
...
Information during the transmission of data
...
Technical Configuration (outside Odoo)
 If your customer or partner need further assistance with that, they need to contact their IT departement or representatives as this is not part of Odoo Configuration.
