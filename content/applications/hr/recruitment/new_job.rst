...
.. note::
   ...
   job position is published, a green :guilabel:`Is Published` toggle is visible. If the job
   position is *not* publishe,d a gray :guilabel:`Not Published` toggle appears. Click the toggle to
   publish or unpublish the job position.
...
.. note::
   Depending on the configuration of the database, and other installed applications, some fields
...
- :guilabel:`Working Schedule`: Select the working schedule for the job position. Odoo provides one
  working schedule by default, :guilabel:`Standard 40 hours/week`, but all available working
  schedules in the database are available.
