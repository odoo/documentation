...
a bill of materials, registering production on a WO automatically unblocks the next WO, making the
next dependent work order ready before the full quantity of the previous is finished.
...
next operation before the full manufacturing order is done. For example, if the first operation has
finished 20 units out of 100, those 20 units can start the next operation while the first work
center continues producing the remaining 80.
...
When working on a work order where only a partial quantity is produced, click the
...
- `16.7` `fluid ounces` `Carbonated Water` (consumed during the mixing stage)
