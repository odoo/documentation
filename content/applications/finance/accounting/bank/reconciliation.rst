...
**Bank reconciliation** is the process of validating your :doc:`bank transactions <transactions>`.
Many of these transactions are matched with counterpart items related to business records such as
...
reconciled, at which point the suspense account is replaced with the account of the counterpart item
...
sequential order to identify a match which is then applied:
...
#. Expand a transaction among unmatched bank transactions to display all available action buttons.
...
If a transaction is for a lower amount than the invoice or bill it is reconciled with, the
transaction is fully reconciled, but the difference remains open on the counterpart item. To
reconcile that remaining amount, :ref:`edit <accounting/reconciliation/edit>` the line, mark it as a
:guilabel:`partial payment`, and reconcile the remaining amount.
...
.. note::
   ...
   original entry automatically posts the reconciliation as well.
...
If no existing item matches the selected transaction, you can still reconcile the transaction
