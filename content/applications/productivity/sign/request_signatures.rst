...
When you need to request a signature from one or more signers, you can initiate a signature request:

- :ref:`from the Sign app <sign/request-signatures/sign-app>`
- :ref:`from any Odoo record <sign/request-signatures/odoo-record>`

In both cases, you can upload one-off documents or :ref:`use templates
<sign/request-signatures/template>` for documents or :ref:`document envelopes
<sign/prepare-document/create-envelope>` that are used regularly.
...
To request the signature of a one-off `.pdf` document:
...
#. :ref:`Prepare the document(s) for signing <sign/prepare-document>` by adding the relevant
   fields, additional signers and/or documents, if relevant.
#. Click :guilabel:`Send`.
...
When using a template, it is possible to:
...
To request the signature of one or more specific signers on the same document:

#. Click :menuselection:`Templates` to see all existing templates.
#. From the kanban or list view, click :guilabel:`Send`. Alternatively, from the kanban view, click
   on a template to open it, then click :guilabel:`Send`.
#. Complete the relevant fields of the :ref:`New Signature Request
   <sign/configure-signature-request>`.
#. If you are:
...
To copy a link to a document or document envelope:
...
When the same single document or document envelope needs to be used regularly, using a template
...
To create a template of a single document or document envelope:
...
With the template open, configure the following:
...
- Optionally, :ref:`define or edit settings for specific signers
  <sign/prepare-document/signer-settings>`, such as assigning a fixed signer, or allowing a signer
  to delegate signing.
- Optionally, define or edit general settings for the template by clicking the :icon:`fa-cog`
  ...
  - :guilabel:`Model`: Define the model the template is linked to. By doing so, the template is
    accessible when :ref:`requesting a signature from a record
    <sign/request-signatures/odoo-record>` of that model.
  ...
  - :guilabel:`Authorized Users`: determine which specific users are authorized to view and use
    this template.
  - :guilabel:`Valid for`: set a number of days after which signature requests using this template
    expire; enter `0` if signature requests should not expire.
...
#. From :menuselection:`Sign --> Documents --> My Documents`, :menuselection:`Sign --> Documents -->
   All Documents`, or :menuselection:`Sign --> Templates`, click the :icon:`fa-cog`
...
To send a signature request from any Odoo record:
...
#. In the window that opens:
   ...
   - to use a :ref:`Template <sign/templates/create>`, select the relevant template from the
     dropdown; or

     .. tip::
        Only templates that are linked to the current model are shown; this is defined in a
        templates :guilabel:`Configuration` settings when :ref:`creating the template
        <sign/templates/create>`.

   - to upload a one-off document or documents, click :guilabel:`Upload PDF` then select the
     relevant `.pdf` document(s) and click :guilabel:`Open`.
...
- **Modify the attributes of the field**: Click on the field to open it, make the necessary changes,
  ...
  - for a :guilabel:`Radio` button field, the number of options: hover over the :guilabel:`Options`
    value then use the :icon:`fa-caret-up` and :icon:`fa-caret-down` icons to increase or decrease
    the number of :guilabel:`Options`. A minimum of two options is required;
...
after the initial upload, and send them for signing in a single signature request. A template can
also contain a document envelope.
...
To adds documents *after* the initial upload:
...
By default, a document or document envelope has one signer. In the :guilabel:`Signers` section in
the left panel, additional :ref:`signers can be added <sign/prepare-document/add-signers>` and
:ref:`advanced configuration can be set <sign/prepare-document/signer-settings>` for one or more
signers.
...
#. Click :guilabel:`Add`.
#. Edit the name of the signer by hovering over the name, e.g., `Signer 2`, then clicking the
   :icon:`fa-pencil` :guilabel:`(pencil)` icon. Type the new name.

.. tip::
   - Signers are assigned a unique color and all fields associated with one signer have the same
     background color.
...
- :guilabel:`Authentication`: Force the signer to :ref:`authenticate using a specific authentication
  method <sign/security/authentication>`.
...
To add :ref:`tags <sign/configuration/tags>` to a document or document envelope *during the
preparation phase or while creating a template*, select the relevant tags from the :guilabel:`Tags`
dropdown in the upper-right corner above the document.

.. tip::
   Tags can also be added to, or removed from a document :ref:`after a signature request has been
   <sign/request-signatures/manage-update-tags>` sent.

.. _sign/configure-signature-request:

Configure a signature request
=============================

After clicking :guilabel:`Send`, the :guilabel:`New Signature Request` window appears.

.. image:: request_signatures/signature-request.png
   :alt: Configuring a signature request
...
- :ref:`set a Valid Until date <sign/configure-request/validity>` after which the signature request
  expires and :ref:`set Reminders <sign/configure-request/validity>` at fixed intervals
- enable :guilabel:`Add certificate on each page` to add the certificate of completion reference to
  each page of the document(s)
- enter one or more contacts in the :guilabel:`CC` field who should be notified when the document is
  fully signed
...
receives the request only once the previous recipient has completed their action.
...
For documents that need to be signed withing a certain timeframe, indicate the last possible signing
date using the :guilabel:`Valid Until` field when configuring the signature request. After this
date, the document is no longer accessible via the link in the signature request email.
...
If the signature request does not expire, leave this field blank.
  ...
  .. tip::
     When using a :ref:`template with a defined validity period <sign/templates/create>`, the
     :guilabel:`Valid Until` date is updated automatically; this date can be changed when
     configuring the signature request if desired.

To set automatic reminders, enable :guilabel:`Reminders` then define the interval, in days, at which
reminders should be sent until the document has been fully signed by the signer.
...
- see the status of individual signers, per signature request:
  ...
  - when a signature request is cancelled, a red :icon:`fa-times` :guilabel:`(cross)` appears beside
    the signers name, which is in red and followed by the date on which the signature request was
    cancelled
- any tags assigned to the document(s)
...
- click the :icon:`fa-ellipsis-v` :guilabel:`(vertical ellipsis)` icon in the top-right of the card
  ...
  - see the :guilabel:`Details` of the signature request
...
#. Go to :menuselection:`Sign --> Documents --> All Documents`, the switch to the
