...
After installing the **Website** app, Odoo prompts you to use its website builder to create the
website. You can:

#. Define the purpose of your website.
#. :guilabel:`Choose a preset palette` or :icon:`fa-cloud-upload` :guilabel:`Use your Logo colors`
   to detect a matching color scheme and generate a brand style accordingly.
#. :guilabel:`Choose your favorite Layout`. You can still change and :doc:`customize
...
.. _prestashop-api-key: https://devdocs.prestashop-project.org/9/webservice/tutorials/creating-access/

.. _ghost-api-key: https://docs.ghost.org/content-api#key

To import your current website, follow these steps:

#. Navigate to :menuselection:`Website --> Configuration --> Settings`.
#. In the settings banner, click :icon:`fa-download` :guilabel:`Import Website`.
#. In the :guilabel:`Import Your Pages and Products` pop-up, enter the :guilabel:`Website URL`
   (e.g., `https://www.example.com`).
#. Ensure :guilabel:`Import Website` is enabled.
...
#. If applicable, complete additional configuration steps for importing products. Odoo automatically
   ...
   `Shopify <https://www.shopify.com/>`_, `WooCommerce <https://woocommerce.com/>`_, or `PrestaShop
   <https://prestashop.com/>`_.
   ...
   .. tabs::
      ...
      .. tab:: WooCommerce
         ...
         in the :guilabel:`Import Your Pages and Products` pop-up:
      ...
      .. tab:: PrestaShop
         ...
         Importing products from `PrestaShop <https://prestashop.com/>`_ requires an API key. Once a
         PrestaShop website is detected, follow these steps:

         #. `Log in to your PrestaShop account <https://authv2.prestashop.com/login>`_ and navigate
            to the PrestaShop back office.
         #. `Create an API key <prestashop-api-key_>`_.
         #. In the PrestaShop user interface, go to :guilabel:`Permissions` and ensure the following
            required permissions are enabled. If any are missing, data transfer may not work as
            expected.
         ...
         #. In the :guilabel:`Import Your Pages and Products` pop-up, paste the key into the
            :guilabel:`PrestaShop API Key` field.

#. When running a blog, enable :guilabel:`Import Blogs` to extract blog post data.
#. If applicable, complete additional configuration steps for importing blog posts. Odoo
   automatically detects the blog platform used by the original website. Blog extraction is
   supported only for websites that use `WordPress <https://wordpress.com/>`_ or `Ghost
   <https://ghost.org/>`_.
   ...
   .. tabs::
      ...
      .. tab:: WordPress
         ...
         `WordPress <https://wordpress.com/>`_ provides a public API that allows Odoo to extract
         blog data without any additional configuration.

      .. tab:: Ghost

         Importing blog posts from `Ghost <https://ghost.org/>`_ requires an API key. Once a Ghost
         website is detected, follow these steps:

         #. `Log in to your Ghost Admin account <https://account.ghost.org/signin>`_.
         #. Go to the :guilabel:`Advanced` section and `create an API key <ghost-api-key_>`_.
         #. Copy the generated :guilabel:`Content API key`.
         #. In the :guilabel:`Import Your Pages and Products` pop-up, paste the key into the
            :guilabel:`Ghost API Key` field.

#. If you want to import products or blog posts to an existing website, i.e., without importing an
   external website, choose :guilabel:`All` or an existing website in the :guilabel:`Website` field.
...
number of pages, products, and blog posts imported. You can then further customize the website using
the :doc:`website editor <web_design>`.
...
.. note::
   ...
   - Importing blog posts automatically installs the :doc:`Blogs app <../blog>` if it is not
     already installed.
   - The tool supports importing up to **100,000** blog posts.
