...
Next, navigate to :menuselection:`Inventory app --> Configfuration --> Locations`. Click
...
Products`. By default, the *Rental* filter appears in the search bar, and the view is Kanban.
...
Navigate to the desired invoice by navigating to **Rental** app and on the Rental Orders dashboard,
in the :guilabel:`INVOICE STATUS` section, click :guilabel:`To Invoice` to view all the rental
orders that require to be sent.
...
:guilabel:`Pickup`. When the *Pickup* page appears, click :guilabel:`Validate`.
...
When a customer returns the products, navigate to the desired rental order, click
:guilabel:`Return`. The *Return* page appears.
...
Click :guilabel:`Validate` and the process is done. A :guilabel:`Returned` status banner appears on
the rental order.
