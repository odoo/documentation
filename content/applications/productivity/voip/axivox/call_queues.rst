...
- **Strategy**: How calls are routed. Choose the option that best matches the company's needs for
  this call queue:
...
- **Maximum waiting time in seconds**: How long a customer can wait in the queue before getting
  forwarded to voicemail or a specific agent.
- **Maximum duration of ringing at an agent**: How long an individual agent's phone rings before the
  call gets moved onto the next step in the dial plan. Learn more about :doc:`dial plans
  <dial_plan_basics>`.
