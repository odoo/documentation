...
- **EDI**: Stands for *Electronic Data Interchange*, which refers to the sending of electronic
  documents.
...
:menuselection:`Accounting  --> Configuration --> Settings`, and scroll to the :guilabel:`Ecuadorian
...
.. note::
   - The values entered in the :guilabel:`Consumables` and :guilabel:`Services` withholding fields
     are used as default values for domestic **only when** no withholdings are set up on the
     *SRI Taxpayer Type*.
...
.. note::
   This configuration applies only if the company is recognized as a withholding agent by the SRI.
...
.. tip::
   If the :guilabel:`Taxpayer Type` is :guilabel:`Rimpe`, configure the :guilabel:`Profit
   Withhold` percentage.
...
- :guilabel:`Journal Name`: Enter in this format: `[Emission Entity]-[Emission Point]
  [Document Type]`, e.g., `001-001 Sales Documents`.
...
- :guilabel:`Use Documents?`: Enable this option if legal invoicing (invoices, debit/credit notes)
  is used, as this is the standard configuration. If not, select the option that will be used to
  record accounting entries unrelated to legal invoicing documents, such as receipts, tax payments,
  or journal entries.
- :guilabel:`Emission Entity`: Enter the establishment number.
- :guilabel:`Emission Point`: Enter the printer point.
- :guilabel:`Emission address`: Enter the address of the establishment.
...
- :guilabel:`Tax Support`: Configure only for the IVA tax. This option is used for registering
  purchase withholdings.
- :guilabel:`Code ATS`: Configure only for income tax withholding codes, as it is necessary for
  registering a withholding.
...
Document types can be accessed or configured by going to :menuselection:`Accounting -->
...
- :guilabel:`Reversal Date`: Enter the date.

Then, click :guilabel:`Reverse`.

When the :guilabel:`Partial Refund` option is used, the amount of the credit note can be changed and
validated. Make sure to specify the relevant product with the correct taxes for a partial refund.

Customer debit note
~~~~~~~~~~~~~~~~~~~

The :guilabel:`Customer debit note` is an electronic document that, when validated, is sent to SRI.
A validated (posted) invoice is necessary to register a debit note. On the invoice, click
:guilabel:`Debit note` and complete the following information in the :guilabel:`Create debit note`
window.
...
.. important::
   ...
   need to edit the amount of the tax in the :guilabel:`Vendor bill`, click :guilabel:`Edit`.
   Or, from the :guilabel:`Journal Items` tab, click :guilabel:`Edit` and set the adjustment as
   desired.
...
- Foreign companies provided services without residency or establishment in Ecuador.
...
In this case, a purchase liquidation journal must be created.
...
An invoice in a validated state is necessary before registering a :guilabel:`Purchas withholding`.
...
.. example::
   ...
   added in a new line with the same tax support. Odoo will allow it as long as the base total
   matches the :guilabel:`Vendor Bill`'s total.
...
To automatically populate the invoice lines, click the :guilabel:`Auto Fill Invoice Lines`. Then,
click :guilabel:`Confirm Vendor Bill` and :guilabel:`Process Now`. The XML and authorization number
for the purchase liquidation are recorded, and the purchase withholding created from this vendor
bill will have the reimbursement information.
...
To enable expense reimbursement for legal entities, make sure a :ref:`vendor bills journal
<l10n_ec/vendor-bills>` has been created.
...
- Automatically generate a valid electronic invoice for Ecuador at the end of the checkout
  process.
...
:menuselection:`Website --> Configuration --> Payment Providers` section. Then, in the
:guilabel:`Activate Payments` section, click :guilabel:`View other providers`.

Each :doc:`payment provider <../payment_providers>` can be configured from here by selecting a
provider record.
