...
which can be sent by email and via Peppol.
...
#. In the popup window, click :guilabel:`Purchases Journal`.
#. Enter a :guilabel:`Journal Name`.
...
.. note::
   ...
   - Not every Peppol access point supports self-billing. Check with your suppliers to ensure theirs
     does.

To send multiple self-bills at once, select them in the journal's list view and click
:guilabel:`Send`. In the popup window that notes how many self-bills will be sent by each method,
click :guilabel:`Send`.

.. important::
   The :guilabel:`Send` button only appears when accessing the list view of the self-billing journal
   from the :guilabel:`Accounting Dashboard`; it does not appear when accessing the general vendor
   bills list view from the menu.

.. note::
   When sending in bulk, the sending method is determined by the vendor's :ref:`invoice sending
   preferences <accounting/invoice/sending>`.
