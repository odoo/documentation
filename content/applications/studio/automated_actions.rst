...
.. tip::
   - Give automation rules clear names and use the :guilabel:`Notes` section to document their
     purpose and functioning. This makes the rules easier to maintain and enhances collaboration
     between users.
...
.. tip::
   Setting a :guilabel:`Before Update Domain` allows you to specify conditions that must be met
   *before* the automation rule is triggered; this also lets you exclude records that will not be
   impacted, which can enhance performance. In contrast, the conditions defined using the
   ...
   For example, if you want the automated action to happen when an email address is set for the
   first time (in contrast to modifying an email address) on an existing contact that is an
   individual rather than a company, use :guilabel:`Email is not set` and :guilabel:`Is a Company is
   not set` as the :guilabel:`Before Update Domain` and :guilabel:`Email is set` as the
   :guilabel:`Apply on` domain.
