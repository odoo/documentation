...
Settings`, then select :guilabel:`Lots & Serial Numbers`.
...
.. important::
   :guilabel:`Track Inventory` must be enabled for the product to follow the process outlined in
   this document. It is not possible to add inventory tracking by lot or serial number if the
   product is untracked and then purchased or sold. Only unused products can be tracked.
