...
This command loads and activates one of the available languages into the Odoo database.
...
on your PostgreSQL instance, using the :ref:`reference/cmdline/db/init` command.
...
This command is a quick and easy way to obfuscate some of the data in your Odoo
instance, mainly used for instructional purposes or to make quick videos for
the support team helping technicians avoid leaking information.

.. warning::

   Use this command carefully as it is **not** considered a safe way to fully anonymize
   data for transfer to a third party. Images, PDF attachments, amounts, many other
   informations may not be obfuscated and cause sensible information leaks. Please
   carefully review what you share before you do, to remove any sensible information.

Obfuscation is symmetric, so you will also be able to unobfuscate content using the
same password.
...
.. option:: --vacuum
   ...
   Vacuum database after unobfuscating.
...
This commands enables deploying a local module to a remote Odoo instance.
It is mainly used by technical people, internal teams and partners to act
on remote instances of their customers, as it does not require full access
to the machine that hosts the Odoo instance, only the Odoo administration
credentials.
