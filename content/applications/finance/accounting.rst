...
Odoo :guilabel:`Invoicing` is a standalone app designed to create invoices, send them to customers,
and manage payments. It also handles flows involving vendor bills. On the other hand, the
:guilabel:`Accounting` app is a comprehensive accounting solution that allows the same actions and
includes additional features such as standard financial reports, bank reconciliation, budgets, asset
management, and more.
...
.. note::
   :doc:`Fiscal localizations <fiscal_localizations>` is set on the parent company.
...
<accounting/reporting/tax_returns>` as branches may share the parent company’s VAT number or have
their own.
