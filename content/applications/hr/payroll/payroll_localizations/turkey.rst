...
section <payroll/turkey_contract>` of the payroll tab, the net salary is specified in the
:guilabel:`Net Wage` field.

For these employees, the monthly net wage is fixed, with a varying gross amount. The gross salary is
calculated by applying all mandatory deductions, such as income tax, social security contributions,
unemployment insurance, and any employer liabilities. This determines the employee's *net amount*.
...
The gross from net is first determined by the salary for the given net, then applying all mandatory
deductions, such as income tax, social security contributions, unemployment insurance, and any
employer liabilities. This determines the employee's *gross amount*.

The net from gross amount is reverse-calculated based on the *expected net* value, which is affected
by all expected allowances and deductions. Unexpected items, such as voluntary deductions and salary
attachments, are then applied to the *expected net* to calculate the *actual net*.
