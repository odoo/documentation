...
.. tip::
   ...
   right side of the screen. Later, when creating or editing a report, you can also see a preview
   by clicking :guilabel:`Print preview` in the left pane.
...
- :guilabel:`Address`: The company name and address are displayed in the header of :ref:`external
  reports <studio/pdf-reports/header-footer/external>`. You can add multiple lines of text.

.. _studio/pdf-reports/default-layout/tagline:

- :guilabel:`Tagline`: This is displayed in the header of :ref:`external reports
  <studio/pdf-reports/header-footer/external>` using the Light, Striped, Bubble, Wave and Folder
...
- :guilabel:`Paper format`: This defines the default paper size of reports. You can select
  ...
  :ref:`Studio <studio/pdf-reports/create>`.

  .. note::
     Other paper formats may be available depending on localization settings.
...
To create a new report for a :doc:`model </applications/studio/models_modules_apps>`, (e.g., sales orders) access
the model, click the **Toggle Studio** button, then click :guilabel:`Reports`. Click :guilabel:`New`
and, in the popup window that opens, select the type of report. This is solely used to determine
what is displayed in the header and footer:
...
- Change the :guilabel:`Report name`. The new name is applied everywhere (in Studio, in the
  :guilabel:`Print` menu under the :icon:`fa-cog` icon in the record, and in the PDF file name).
...
- Dynamic tables, which are used to display data from
...
You can also add tables by :ref:`modifying the report's XML <studio/pdf-reports/XML-editing>`.
...
Insert the :ref:`field of your choice <studio/pdf-reports/add-field>` in a cell by typing `/` or add
static text by typing.

.. tip::
   To add text in a structured way without using a table, you can use columns. Add columns by
   typing `/` and choosing whether to have two, three or four columns.
...
:ref:`static table <studio/pdf-reports/static-table>`. It is also possible to insert rows that will
appear above or below the generated rows.
...
Insert the :ref:`field of your choice <studio/pdf-reports/add-field>` by typing `/`. The dialog box
that opens shows the source object for the field (e.g., the *Order Lines* model) and the list of
available fields. Select or search for the field and press `Enter`.

.. image:: pdf_reports/available-fields.png
   :alt: List of available fields for the sale order lines model.

Edit the :guilabel:`Column name` by selecting it then typing a label.
