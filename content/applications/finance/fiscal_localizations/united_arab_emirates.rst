...
The United Arab Emirates fiscal localisation provides a fully configured accounting setup aligned
with national VAT and corporate tax regulations. It includes a localised chart of accounts,
VAT-compliant tax groups, standard journals, fiscal year configurations, and reporting features that
ensure accuracy and compliance with the Federal Tax Authority (FTA).

The United Arab Emirates localization package provides the following key features to ensure
...
- :ref:`VAT return <localizations/uae/tax-return>`: structured according to the United Arab Emirates
  VAT return format, as issued by the :abbr:`FTA (Federal Tax Authority)`
- :ref:`Corporate Tax Report <localizations/uae/corporate-tax-report>`: structured according to the
  United Arab Emirates Corporate Tax Report, as issued by the :abbr:`FTA (Federal Tax Authority)`
...
The UAE :doc:`chart of accounts <../accounting/get_started/chart_of_accounts>` is structured in
accordance with :abbr:`IFRS (International Financial Reporting Standards)` and :abbr:`FTA (Federal
Tax Authority)` tax requirements. This includes classified accounts for assets, liabilities, equity,
income, and expenses, as well as separate accounts for VAT payable and VAT receivable.
...
The United Arab Emirates localization includes preconfigured VAT rates and :doc:`taxes
<../accounting/taxes>` as defined by the :abbr:`FTA (Federal Tax Authority)`:
...
- | Out of Scope: Transactions that are not subject to UAE VAT legislation.
...
- :guilabel:`Tax Invoices (B2B)`: A :guilabel:`Tax Invoice`, required for business-to-business
  transactions, contains all the necessary details for VAT recovery, including the supplier and
  customer legal names, addresses, TRN numbers (where applicable), itemized VAT lines, and total
  amounts.
- :guilabel:`Simplified Tax Invoices (B2C)`: A :guilabel:`Simplified Tax Invoice` is primarily
  intended for consumer sales (B2C). It includes less information because the customer is not
  expected to reclaim VAT. Typically, it does not require a customer :abbr:`TRN (Tax Registration
  Number)` and may display limited customer details, focusing instead on supplier information, the
  VAT charged, and total amount due.

.. important::
   A :guilabel:`Simplified Tax Invoice` is still considered an invoice under VAT law and differs
   from a retail receipt.
