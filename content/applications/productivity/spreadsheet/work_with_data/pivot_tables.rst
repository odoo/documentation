...
- :guilabel:`Display options`: define the maximum number of rows or columns to show, or indicate if
  the totals, column titles, and/or measure titles are shown. It is also possible to :ref:`flatten
  hierarchical data <spreadsheet/pivot-tables/dimensions-add-flatten>` by enabling :guilabel:`Use
  tabular form`.

  .. tip::
     Many display options of a :ref:`dynamic pivot table
...
.. _spreadsheet/pivot-tables/dimensions-add-flatten:

.. tip::
   To flatten a pivot table, i.e., convert hierarchical data into a simple table where each row
   dimension field has its own column and every line has complete information with repeated labels,
   enable :guilabel:`Use tabular form` in the :guilabel:`Design` tab of the :ref:`pivot table's
   properties panel <spreadsheet/pivot-tables/properties-design>`.

   .. example::
      In the example, with tabular form enabled, the row dimensions :guilabel:`Salesperson` and
      :guilabel:`Product` are no longer nested in a single column, but are presented in separate
      columns.

      .. image:: pivot_tables/tabular-form.gif
         :alt: Pivot table in tabular vs compact form
