...
- :icon:`fa-truck` :guilabel:`Delivery`: Linked to the **Inventory** app and shows any delivery and
  warehouse receipts related to the rental order.

.. important::
   For the appropriate smart buttons to display, the **Project**, **Timesheet**, **Planning**, and
   **Inventory** apps are needed. The selected :ref:`physical
   <rental/service_products/configuration>` or :ref:`labor
   <rental/labor_service_products/configuration>` service product on the rental order must be
   properly configured to integrate with the recommended apps.
...
In the *New Signature Request* pop-up window, either :ref:`select an existing document template
...
Click the :icon:`fa-puzzle-piece` :guilabel:`Tasks` smart button on the top of the rental order to
...
in the *Invoice Status* section in the left navigation bar, click :guilabel:`To Invoice` to view all
rental orders that need to be sent.
