...
Select a stage to fold temporarily, then click the :icon:`fa-cog` :guilabel:`Actions` icon, and
...
Identify the tickets to be merged and tick the checkbox at the far left of each ticket. Then, click
the :icon:`fa-cog` :guilabel:`Actions` icon, and select :guilabel:`Merge` from the drop-down menu.
Doing so opens a new page where the selected tickets are listed with their :guilabel:`Similarity`
rating. From here, tickets can be :ref:`merged <data_cleaning/merge-records>` or
:guilabel:`Discarded`, as desired.
