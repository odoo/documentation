...
#. In Odoo, go to :menuselection:`Website --> Configuration --> Settings`.
#. In the :guilabel:`Tracking & SEO` section, enable :guilabel:`Google Tag Manager/Google
   Analytics`, paste the measurement ID in the :guilabel:`Identifier` field, and click
   :guilabel:`Save`.
...
#. `Create an account and a container in Google Tag Manager
   <https://support.google.com/tagmanager/answer/14842164?hl=en>`_.
#. On the `GTM accounts page <https://tagmanager.google.com/#/home>`_, locate the relevant website
   and copy the container ID from the :guilabel:`Container ID` column. The ID starts with `GTM-`,
   e.g., `GTM-1234XXX4321`.

   .. image:: analytics/GTM-container-id.png
      :alt: Locate the container id.

#. In Odoo, go to :menuselection:`Website --> Configuration --> Settings`.
#. In the :guilabel:`Tracking & SEO` section, enable :guilabel:`Google Tag Manager/Google
   Analytics`, paste the container ID in the :guilabel:`Identifier` field, and click
   :guilabel:`Save`.

.. tip::
   Alternatively, GTM can be configured by manually inserting code snippets. To do so:

   #. On the `GTM accounts page <https://tagmanager.google.com/#/home>`_, click the relevant
      website.
   #. On the :guilabel:`Workspace` page, click the container ID in the upper-right corner.
   #. Copy the `<head>` and `<body>` code from the pop-up window.
   #. In Odoo, go to your website, click :guilabel:`Edit`. Then, navigate to the :guilabel:`Theme`
      tab, scroll down to the :guilabel:`Advanced` section, and, next to :guilabel:`Code Injection`,
      click :guilabel:`<head> and </body>` to paste the corresponding code.
   #. Click :guilabel:`Save`.
