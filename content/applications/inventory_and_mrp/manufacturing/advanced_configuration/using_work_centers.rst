...
.. note::
   By default, Odoo uses the `Work Center 40 hours/week` working hours, which expects work centers
   to operate between 8:00 AM and 4:00 PM on Monday through Friday.
...
The currently scheduled work orders can be viewed by accessing the planning views from the
:menuselection:`Manufacturing app --> Planning` pages.

.. image:: using_work_centers/work-center-planning.png
   :alt: The Gantt view of planning showing two assembly work centers.

Different pages show how many individual work orders are scheduled, how hours the work center is in
production, and the dates and times that are currently scheduled for work orders. Times and planned
work centers can be changed by clicking to access the individual work order.

.. seealso::
   :ref:`manufacturing/work_order_dependencies/planning-views`
...
.. example::
   ...
   click to remove the :icon:`fa-filter` **To Do** *or* **Blocked** *or* **In Progress** filter by
   clicking the :icon:`oi-close` :guilabel:`(Remove)` icon on its right side. Next, click the
   measures button and select :guilabel:`Duration Deviation (%)`.
