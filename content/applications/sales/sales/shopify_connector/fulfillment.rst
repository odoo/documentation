...
Configuring deliveries to be handled in Odoo is useful when a company's warehouse team is
...
The warehouse delivery form is created in the :guilabel:`Ready` stage in Odoo. Click
...
every 10 minutes pushes the shipment to Shopify. Alternatively, click :guilabel:`Update to
E-commerce` to push the update manually.

.. image:: fulfillment/warehouse-receipt-odoo.png
   :alt: Example of a warehouse transfer when Odoo is configured to handle deliveries.

If the delivery successfully updates to Shopify, a green :guilabel:`Synchronized with E-commerce
Platform` badge displays, and a log note with a success message is added to the delivery form. If
the delivery fails to update, a red :guilabel:`Synchronization with E-commerce Platform failed`
badge displays, and a log note with a failure message is added to the delivery form.
...
within Shopify or by a third-party logistics provider. When the Shopify account form is set to
...
.. image:: fulfillment/warehouse-receipt-shopify.png
   :alt: Example of a warehouse transfer when Shopify is configured to handle deliveries.
...
delivered order is then pulled into Odoo, its warehouse delivery form updates to the
:guilabel:`Done` stage. Similarly, if a pulled order already has the *Fulfilled* tag, Odoo creates
its warehouse delivery form directly in the :guilabel:`Done` stage.

Corresponding warehouse delivery forms are created as a backorder of the default picking, to reduce
...
- Fulfillments from different locations.
...
When a Shopify order includes a shipping fee and carrier, Odoo adds the default
:guilabel:`E-commerce Shipping` product to the invoice and order with the shipping fee. The actual
name of the delivery carrier selected by the customer (such as Sendcloud, Shiprocket, or FedEx) is
added to the description of that invoice or order line.
...
To map a Shopify store's stock location to an Odoo stock location, refer to the
...
Configuration --> Shopify: Locations` or click the :icon:`fa-cubes` :guilabel:`Locations` smart
...
To edit the stock location mapping, click the :guilabel:`Stock Location` field and select an option
...
The Shopify connector uses scheduled actions to sync orders, deliveries, and stock between Shopify
and Odoo. Deliveries received from Shopify are processed using a backorder-based approach: partial
...
Refer to the :doc:`manage` page for more information on order syncing. For information on inventory
syncing with returns, refer to the :ref:`shopify/fulfillment/returns` section.
...
The :guilabel:`Create Invoice` option is available on the :ref:`Configuration tab
<shopify/setup/administrative-settings>` of the Shopify account form. This option controls whether
Odoo automatically creates invoices when importing orders from Shopify.
...
Also in the *Configuration* tab, there is a *Journals* section that allows configuring accounting
...
applicable). This sync is ongoing: if a return or refund is created on Shopify after the order has
already been fetched, it is still synced when the order is fetched again.

During the sync, Odoo first checks whether a return or refund already exists on the :abbr:`SO (sales
order)` using the unique identifiers provided by Shopify. Only returns and refunds that do not yet
exist in Odoo are synced.
...
On the *Process return* page, verify the return item information and select a
:guilabel:`Restock at` option. Then, click :guilabel:`Process and refund` again to confirm and close
the return in Shopify.
...
the :icon:`fa-truck` :guilabel:`Delivery` smart button to view the *Transfers* page which lists all
stock movements for the :abbr:`SO (sales order)`. Click the warehouse receipt entry with a `Return
of WH/OUT/reference number` in the :guilabel:`Source Document` column.

.. image:: fulfillment/return-transfers-page.png
   :alt: Example of the Transfers page for a Shopify SO in the Sales app.

The warehouse receipt is always created in the :guilabel:`Ready` stage, so that the user can review
it before validation. On the form, the return date from Shopify is set as the :guilabel:`Scheduled
Date`. The :guilabel:`E-commerce Return Identifier` field displays the unique return identifier
number that Shopify added for traceability and to avoid duplication during synchronization. The
warehouse receipt form lists the return products and their quantities on their own order lines.
Click :guilabel:`Validate` when the return product is received. The stage on the warehouse receipt
form changes to :guilabel:`Done`.
...
Go back to the :abbr:`SO (sales order)` by clicking the SO number in the breadcrumbs. Click the
:icon:`fa-pencil-square-o` :guilabel:`Invoices` smart button. The credit note line item is
highlighted in blue and shows a negative amount in the :guilabel:`Total` column. Click the credit
...
The credit note is always created in the :guilabel:`Draft` stage to allow for user review. On the
credit note form, Shopify sets the refund date as the :guilabel:`Invoice Date`. The
:guilabel:`E-commerce Refund Identifier` field displays the unique refund identifier number that
Shopify added for traceability and to prevent duplication during synchronization.
...
note. The status is updated to :guilabel:`Posted`. Then, click :guilabel:`Pay`, and in the *Pay*
pop-up window, click :guilabel:`Create Payment` to complete the refund. The credit note form
displays a :guilabel:`Paid` banner.
...
.. warning::
   ...
   only shows credit notes made against an invoice line of the sales order. Refunds using the refund
   adjustment product apply to the whole invoice, rather than a specific invoice line. To view the
   refund, users must go to :menuselection:`Invoicing app --> Customers --> Refunds`.

The Shopify connector automatically syncs refund data for orders in Shopify accounts with
...
*Posted* stage. If these conditions are not met, the refund is skipped.
...
On the :abbr:`SO (sales order)`, there are :icon:`fa-truck` :guilabel:`Delivery` and
:icon:`fa-pencil-square-o` :guilabel:`Invoices` smart buttons. The :doc:`fiscal position
...
Click the :icon:`fa-pencil-square-o` :guilabel:`Invoices` smart button. The credit note line item is
highlighted in blue and shows a negative amount in the :guilabel:`Total` column. Click the credit
note to open the form.
...
Optional: Canceling a delivery associated with a refund
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

When a customer requests a refund before the order is shipped, the order sync still generates an
invoice and a warehouse delivery form in Odoo. Users must manually cancel the delivery form when
processing the refund.

Go to :menuselection:`Sales app --> Configuration --> Shopify: Accounts` and select the desired
Shopify account. Click the :icon:`fa-dollar` :guilabel:`Orders` smart button and select the desired
:abbr:`SO (sales order)`. Click the :icon:`fa-truck` :guilabel:`Delivery` smart button, then click
:guilabel:`Cancel` on the warehouse delivery form.
