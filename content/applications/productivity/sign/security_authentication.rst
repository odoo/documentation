...
Each time a document or document envelope is fully signed, i.e., signed by all signers, a
certificate of completion is generated and sent to all signers via email, along with the fully
signed document(s).

.. note::
   When documents are :doc:`signed via a record's chatter <sign_document>`, or when a
