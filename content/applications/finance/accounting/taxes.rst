...
scroll down to the :guilabel:`Taxes` section, select the appropriate taxes for your default sales
tax and purchase tax in the :guilabel:`Default Taxes` field, and click on :guilabel:`Save`.
...
.. note::
   Since the first fiscal position in the sequence is considered the company's default fiscal
   position, the taxes set on products are expected to be used with that fiscal position, so the
   :guilabel:`Replaces` field is not displayed on that fiscal position.
