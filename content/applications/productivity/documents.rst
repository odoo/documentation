...
- :icon:`fa-building` :guilabel:`Company`: contains folders and files shared across the company.
  Access is determined by the user's :ref:`assigned rights <documents/access-rights>`.
...
Click a file to open it and :ref:`perform any necessary actions <documents/documents>`. To close
the file, press **Esc** or click the :icon:`fa-remove` (:guilabel:`close`) icon. You can also drag
and drop a file or folder to move it to another folder or section.
...
To enable the centralization of files attached to a specific area of your activity, go to
:menuselection:`Documents --> Configuration --> Settings`. For example, enabling :guilabel:`Human
Resources` makes HR documents automatically available in the HR folder, while documents
related to Payroll are automatically available in the Payroll sub-folder. Select the desired
folder from the dropdown list and select the :guilabel:`Tags` to be added to the relevant files.

.. note::
   - When centralizing accounting files, click :guilabel:`Journals` and define each journal
     individually to enable automatic synchronization.
...
- :icon:`fa-external-link-square` :guilabel:`Add shortcut`: This option is only available for
  sub-folders and allows to create a shortcut to a sub-folder. If you have editing permissions, the
  shortcut is created in the same folder; otherwise, it appears in the :icon:`fa-hdd-o`
  :guilabel:`My Drive` section. You can then drag and drop it into the desired folder.
- :icon:`fa-star-o` :guilabel:`Add star`: Mark a folder as a favorite for quicker access. This
  ...
  :guilabel:`Starred` filter to navigate to your favorite folders quickly.
- :icon:`fa-info-circle` :guilabel:`Info & Tags`: View the folder's :ref:`details
  <documents/details-panel>`.
...
- :icon:`fa-external-link-square` :guilabel:`Create shortcut`: A shortcut is a pointer to a file,
  allowing access from multiple folders without duplicating the file. If you have editing
  :ref:`permissions <documents/access-rights>`, the shortcut is created in the same folder;
  otherwise, it appears in the :icon:`fa-hdd-o` :guilabel:`My Drive` section. You can then drag and
  drop it into the desired folder.
...
To see the list of all requested files, switch to the :guilabel:`Activity` view  and go to the
:guilabel:`Requested Document` column. Click a requested file's date to view its details.
You can then:
...
In the pop-up, grant access to specific users or contacts by selecting their name from the
...
the :icon:`fa-info-circle` (:guilabel:`Info & Tags`) in the upper-right corner next to the view
icons.
...
- Change the file's or folder's :guilabel:`Owner` and :guilabel:`Contact`. By default, the
  ...
  .. note::
     To view a file in :guilabel:`My Profile`, a user must be set as the :guilabel:`Contact`
     and have at least :guilabel:`Viewer` :ref:`access <documents/access-rights>`.
...
You can use an email alias to automatically save files received by email into a specific folder.
To set up an email alias for a folder, follow these steps:
...
To add tags to a file, open the file, click the :icon:`fa-info-circle`
(:guilabel:`Info & Tags`) in the upper-right corner next to the view icons, and then, in the details
panel, select a tag from the dropdown list.

.. note::
  :ref:`Alias tags <documents/email-aliases>` can also be used to automatically apply tags to
  files created through the alias.

Digitize files with AI and optical character recognition (OCR)
==================================================================

Files available in the Finance folder can be digitized. Select the file, click
:guilabel:`Create Vendor Bill`, :guilabel:`Create Customer Invoice`, or
