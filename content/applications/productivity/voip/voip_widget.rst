...
Manually transfering a call only happens when on a call. to transfer a call within the |VOIP|
...
If the user doesn't pick up the call or is busy with another call, then calls can be automatically
transfered. This gets configured with the |VOIP| service provider.
