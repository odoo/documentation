...
.. code-block:: python
    ...
    def _cron_do_something(self, limit=300):  # some default batch limit, allows for tweaking
        ...
        # notify that there are remaining records to be processed
        remaining = 0 if len(records) == limit else self.search_count(domain)
        self._cron_do_something.notify(done=len(records), remaining=remaining)
...
.. code-block:: python
    ...
    def _cron_do_something(self):
        ...
        self._cron_do_something.notify(remaining=len(records))
        ...
        with open_some_connection() as conn:
            def process_record(record):
                ...
                self._cron_do_something.progress()  # consume one remaining
