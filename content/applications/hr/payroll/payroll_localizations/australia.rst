...
- :guilabel:`GST registered`: Tick this bock if the company is registered for Australian :abbr:`GST
...
- :guilabel:`Withholding Payer Number`: If available, enter the eight or nine-digit :abbr:`WPN
  (Withholding Payer Number)` issued by the :abbr:`ATO (Australian Taxation Office)`. This is for
  companies *not* entitled to an :abbr:`ABN (Australian Business Number)`.
- :guilabel:`Registered for Working Holiday Maker`: Tick this box if the company is registered for
  ...
  :abbr:`ATO (Australian Taxation Office)` to withhold the correct taxes for employees working in
  Australia with an eligible visa, for a period of less than 12 months.
...
in the **Settings** app or **Emplyees** app, then return to the **Payroll** settings page. Click the
:guilabel:`Start Payroll Onboarding` button, and the pop-up window loads to the :guilabel:`Employer
Details` again.
...
localization. Additional fields are present after configuring the database for the Australia.
...
- :guilabel:`Wage`: Enter the wage, and select the corresponding time period. The available options
  are :guilabel:`Daily`, :guilabel:`Weekly`, :guilabel:`Fortnighly`, :guilabel:`Monthly`, and
  :guilabel:`Quarterly`.
...
- :guilabel:`Pay Category`: Select :guilabel:`Australia: Employee` for this field. This defines
  when the employee is paid, their default working schedule, and the work entry type it applies to.

.. image:: australia/payroll-overview.png
...
- :guilabel:`Tax File Number`: If :guilabel:`Declaration provided` is selected for the
  ...
  selected, this field autopulates with a placeholder number, and **cannot** be modified.
...
following information in the po-up window:
...
- :guilabel:`Active`: Enable this option if the account is active (this is the liekyl scenario).
...
Parameters specific to the Australia.
...
.. important::
   ...
   *out-of-cycle* runs. The same payslip rules apply, but the way individual payslips are submitted
   to the :abbr:`ATO (Australian Taxation Office)` in the framework of :abbr:`STP (Single Touch
   Payroll)` is slightly different.
...
.. important::
   ...
   employee's bank account is *not* marked as `trusted`, :abbr:`ATO (Australian Taxation Office)`
...
As per :abbr:`ATO (Australian Taxation Office)` requirements, :abbr:`STP (Single Touch Payroll)`
submissions for a pay run **must**** to be done *on or before* the payday. For this reason, submit
:abbr:`STP (Single Touch Payroll)` data to the :abbr:`ATO (Australian Taxation Office)` **before**
proceeding with payment. To submit the data, click :guilabel:`Submit to ATO` on the pay run.

.. tip::
   A warning message appears if important information is missing.

On the :abbr:`STP (Single Touch Payroll)` record for the pay run, a
...
:guilabel:`Employee Finalisation` tab. Ensure all balances for al employees is correct. Once
verified, click :guilabel:`Submit to ATO` to create the report and submit it.

Employees payment information changes to *Tax ready* on their online income statement, after the end
...
.. important::
   If an employee record is finalised partway through the financial year, the :abbr:`ATO (Australian
   Taxation Office)` will **not** pre-fill the information into the employee's tax return until
...
- :guilabel:`Cessation Type Code`: Using the drop-down menu, select the *code* for the termination.
  ...
  - :guilabel:`(R) Redundancy`: Employees wo have been laid-off.
...
:guilabel:`Long Service Leaves` the employee still has. These fields are autopopulated based on
**Time Off** app balances.

When the form is complete, click :guilabel:`Confirm Termination`, and the final payslip for the
employee loads with a red :guilabel:`FINAL PAY` banners appears in the upper-right corner. The
payslip covers the time period from the last payslip to the termination date. After the employee is
terminated, the :guilabel:`Contract End Date` is populated on the employee form.
