...
base account is used: (:guilabel:`899.01.99 Base Imponible de Impuestos en Base a Flujo de
Efectivo`) in the journal entry when reclassifying taxes. **Do not delete this account**.
...
(PAC)` section, enter the name of your |PAC| with your credentials (:guilabel:`PAC username` and
:guilabel:`PAC password`).
...
#. The :guilabel:`Weight` of the product in the :guilabel:`Accounting` tab must be more than `0`.
#. The `correct  <https://www.ventanillaunica.gob.mx/vucem/Clasificador.html>`_ :guilabel:`Tariff
   Fraction` of the product must be set in the :guilabel:`Accounting` tab.
...
require converting the amounts of your product into USD. Therefore, :doc:`multicurrency
...
:guilabel:`Send` button to sign it.
...
:guilabel:`All active orders` filter to :guilabel:`Paid`, select the order, and tick the
:icon:`fa-file-text-o` :guilabel:`Invoice` checkbox.
