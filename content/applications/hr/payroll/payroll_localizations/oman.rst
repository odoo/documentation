...
.. list-table::
   ...
   * - :guilabel:`Oman - Payroll`
     ...
     - Includes all salary rules, leave logic, end of service, |WPS|, and compensation rules
       compliant with Oman Labor Law.
...
.. _payroll/oman/payroll_localizations/om_gen:
...
.. _payroll/oman/payroll_localizations/om_payroll:
...
- :guilabel:`WPS Disbursement Bank Account`: Using the drop-down menu, select the bank account used
  ...
  .. important::
     ...
     - :guilabel:`BIC/SWIFT`: Enter an 8 or 11-character alphanumeric code, **not** a 9-digit
       numeric value.
...
.. _payroll/oman/payroll_localizations/om-personal_tab:
...
.. _payroll/oman/payroll_localizations/om-payroll_tab:
...
- *Sick leave*: The rate at which employees in Oman are paid for sick days depends on the number of
...
on the :ref:`employee profile <payroll/oman/payroll_localizations/om-payroll_tab>`, the employee is
...
- *Unused annual leaves compensation*: The *Annual Leave Balance* is shown on the employee record.
  ...
  .. important::
     ...
     *deserved leaves*, which are defined by the period of time the employee worked during the
     current year.
...
monthly basis but does **not** affect the employee's net wage.
...
.. _payroll/oman/payroll_localizations/om_config:
...
When the **l10n_om_hr_payroll** module is :ref:`installed <payroll/oman/om_apps>`, a new
:guilabel:`Salary Structure` gets installed, :guilabel:`Oman: Monthly Pay`. This structure includes
one :guilabel:`Structure Type`, :guilabel:`Oman: Employee`.

The :guilabel:`Salary Structure` contains all the individual :ref:`salary rules
<payroll/oman/om_rules>` that inform the **Payroll** app how to calculate employee payslips.

.. image:: oman/om-structure-types.png
   :alt: The Omann salary structure in the salary structures list.

.. _payroll/oman/om_rules:
...
wage amounts are correct. The computation of hours worked reflects the employee's actual hours
worked, among others.

When running a pay run, check that the period, company, and employees included are correct *before*
...
amounts on the right dates. It can either be generated per payslip or pay run.
...
#. Ensure the :guilabel:`Company MOL Number`, :guilabel:`Salary Payer`, :guilabel:`Salary Payer MOL
   ...
   <payroll/oman/payroll_localizations/om_payroll>`.
#. Set the unique identifier for all employees who are part of the pay run or payslip.
   ...
   .. tip::
      ...
      *Personal* tab on the :ref:`employee form
      <payroll/oman/payroll_localizations/om-personal_tab>`.
...
#. Post the draft entry related to the payslips.
...
.. list-table::
   ...
   * - Total Salaries
     - Sum of Net for all payslips in the pay run (or single Net for off-cycle)
