...
criteria for when an approval is required before an action can be performed using a button.

Configuration
=============

To add approval rules, proceed as follow:
...
#. In the :guilabel:`Properties` tab on the left, enable the :guilabel:`Set approval rules`
   feature.
#. Specify the :guilabel:`Approval Group` to limit the permission to approve to a specific user
   group.
...
- When an unauthorized user clicks the button, an error message is displayed on the top-right and
  an activity is created for the user specified in the :guilabel:`Responsible` field.
