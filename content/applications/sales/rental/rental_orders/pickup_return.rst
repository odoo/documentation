...
products in the eCommerce store, refer to the :ref:`Configure pickup and return availability
for eCommerce products <rental/pickup_return/ecommerce-products>` section.
...
The Rental app supports eCommerce configuration at a company and product level when the **Website**
app and the **eCommerce** module are installed. Using the *Availabilities* setting restricts the
days and time the company allows pickup and returns to be scheduled through the online store. To
configure the rental product page in the online store, refer to the :ref:`Product configuration
...
To configure default settings for pickup and return availability of rental products through in
eCommerce store, navigate to :menuselection:`Rental app --> Configuration --> Settings`.

In the *Pickup & Returns* section, click the :guilabel:`Availabilities` field and select a rental
work calendar from the drop-down menu to set which days and business hours rental products can be
picked up and returned online. The :guilabel:`Availabilities` drop-down menu offers preconfigured
work calendar options. To edit the default calendar, click the :icon:`fa-arrow-right`
:guilabel:`(Internal Link)` icon.

.. image:: pickup_return/availabilities-setting.png
   :alt: Example of the Availabilities setting on the Setting page of the Rental app.

To create a new work calendar, click the :guilabel:`Availabilities` field, then click the
:guilabel:`Search more` option. A *Search: Rental Calendar* pop-up window displays with
preconfigured work calendar entries. Two columns organize the pop-up window:

- :guilabel:`Working Time`: How many hours per week.
- :guilabel:`Work time Rate`: The Work Time rate versus the full-time working schedule.

.. image:: pickup_return/search-rental-calendar-window.png
   :alt: Example of the Search: Rental Calendar pop-up window in the Rental app.

Click :guilabel:`Create New`, and the *Create Rental Calendar* pop-up displays. All work calendars
apply at the company level. In the :guilabel:`Company` field, select the company to which the
calendar applies. For :guilabel:`Calendar Type`, choose either :ref:`Fixed <custom anchor>` or
:ref:`Variable <custom anchor>`.

.. image:: pickup_return/create-rental-calendar-window.png
   :alt: Example of the Create Rental Calendar fields in the Rental app.

*Fixed* calendar types are good for businesses that maintain the same work hours and days each week.
This option allows businesses, such as car or equipment rentals to coordinate duties and maintain
consistent business hours.

*Variable* calendar types are well-suited to businesses with split work schedules. This option
allows businesses that deal with variable demand, such as hotels or tourism-related industries, to
staff peak hours while reducing labor costs during slow periods.
