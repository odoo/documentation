...
.. list-table::
   ...
   * - :guilabel:`United States - Payroll with Accounting`
     ...
     - hr_payroll_account
       l10n_us
       l10n_us_hr_payroll
     - This module links payroll and accounting. Its main purpose is to allow the creation of a
       journal entry (per payslip, if needed) to reflect the impact of payroll on the company's
       books.
...
.. warning::
   Ensure the information is accurate, as it is used when creating W2's.
...
<employees/private-contact>` (a warning appears on the **Payroll** dashboard, and an error message
appears when attempting to run payroll).
...
.. important::
   ...
   edit rule parameters **unless a federal or state parameter has changed**, and is different than
...
from employee paychecks. It also reports the employees' side of taxes.
