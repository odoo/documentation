...
The :guilabel:`Job Posting` section of the **Recruitment** app settings has two configurations to
make, enabling posting jobs on the company website, and on external job boards.
...
Settings`. The corresponding job board credentials is listed. Enter the :guilabel:`Username` and
...
Enable the :guilabel:`Résumé Display` option to show the resumé on the applicant's card by default,
and in addition to the document link. When enabled, the resumé appears on the right side of the
applicant's card.
...
address from the resumé and populate the applicant's form. To do so, enable the :guilabel:`Résumé
...
Service & Buy Credits` intenral link to purchase credits for Résumé digitization. Click the
...
.. note::
   ...
   resumés can be scanned. Disabling this option uninstalls the module.

   If at some point, there is a desire to temporarily stop digitizing resumés, select the
...
If any attachments are to be added, click the :icon:`fa-paperclipsend-survey`
:guilabel:`(paperclip)` button at the bottom of the window. Navigate to the file to be attached,
then click :guilabel:`Open` to attach it. To delete an attachment, click the :icon:`fa-close`
:guilabel:`(delete)` icon to the right of the attachment.
