...
The :guilabel:`Design` tab of the properties panel provides options for how the pivot table is
displayed, such as defining the maximum number of rows or columns to show, or indicating if the
totals, column titles and/or measure titles should be shown.
...
Adding multiple row or column dimensions creates a nested hierarchy. To change the hierarchy
of the dimensions, drag the dimension's card to the desired position within its section.
