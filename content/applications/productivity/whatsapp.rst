...
.. important::
   ...
   To switch to the business account, navigate to the `Facebook Developer Console
   <https://developers.facebook.com>`_ and click the *account name* in the top-right corner. Under
...
Click :guilabel:`My Apps` in the top-right corner after successfully signing in to the Meta
developer account. This redirects the administrator to all the apps the developer has configured in
this specific developer account. Click :guilabel:`Create App` to configure a new Meta application.

Select the app type
-------------------

The first step in app creation is to select the app type. On the :menuselection:`Create an app`
page, select :guilabel:`Looking for something else? --> Other --> Next`, then select the first
option listed under the :guilabel:`Select an app type` label, titled :guilabel:`Business`. This
selection allows for the creation and management of the WhatsApp :abbr:`API (Application Programming
Interface)`. Click :guilabel:`Next` to configure the app as desired.
...
In the :guilabel:`Details` section of the :guilabel:`Create an app` process, enter `Odoo` in the
...
Lastly, use the drop-down menu to set the :guilabel:`Business Account - Optional` field to the Meta
business account profile. Click :guilabel:`Create app` and review the *Meta Platform Terms* and
*Developer Policies* agreements.
...
.. warning::
   If the Meta business account is prohibited from advertising, the app can't be claimed. To
   resolve this issue, navigate to `Meta for Business <https://business.facebook.com/business>`_ for
   assistance.
...
Navigate to the `Meta App Dashboard <https://developers.facebook.com/apps>`_, and click on the app
that is being configured.

On the next page, click :guilabel:`Set up` next to the box containing WhatsApp, located toward the
bottom of the page.
...
API`. Under :guilabel:`Select a Meta Business Account` option, select the Meta business to be
configured, then click :guilabel:`Continue` to confirm the selection and agree to Meta's terms and
conditions as linked on the :guilabel:`Meta App Dashboard`.
...
.. note::
   ...
   navigate to the `Meta App Dashboard <https://developers.facebook.com/apps>`_ and select the
   `Odoo` app.
...
In another browser tab, navigate to `Meta for Developers <https://developers.facebook.com>`_. Select
:menuselection:`My Apps --> WhatsApp --> API Configuration`, and then copy the following values from
the Meta developer console into the corresponding fields in Odoo:
...
To retrieve the :guilabel:`App Secret`, navigate to the `Meta App Dashboard
<https://developers.facebook.com/apps>`_, and select the `Odoo` app. In the left-hand side menu,
...
To configure the webhooks for the Odoo **WhatsApp** app, navigate to the `Meta App Dashboard
<https://developers.facebook.com/apps>`_ and select the `Odoo` app. In the left-hand side menu,
click :guilabel:`WhatsApp --> API Setup`. Go to :guilabel:`Step 3: Configure webhooks to receive
messages` and click :guilabel:`Configure webhooks`.

.. tip::
   Webhook configuration settings can also be accessed by navigating to the `Meta App Dashboard
   <https://developers.facebook.com/apps>`_, selecting the `Odoo` app, and then selecting
...
App Dashboard <https://developers.facebook.com/apps>`_ and select the `Odoo` app. Under
...
Navigate to `Meta Business Tools <https://business.facebook.com/>`_ and then go to
...
Finally, to launch the app, the Meta app must be set to :guilabel:`Live` in the `Meta App Dashboard
<https://developers.facebook.com/apps>`_. Click the app that is being configured, then toggle the
...
.. warning::
   A privacy policy URL must be set in order for the app to be set to live. Go to the `Meta App
   Dashboard <https://developers.facebook.com/apps>`_ and select the `Odoo` app. Then, in the
   left-hand side menu, go to :menuselection:`App Settings --> Basic`. Enter the privacy policy
...
Each template has three tabs: :guilabel:`Body`, :guilabel:`Buttons`, and :guilabel:`Variables`. The
message is stored in the :guilabel:`Body` tab; the message body may contain placeholders for dynamic
content, which is populated when the message is sent. The :guilabel:`Variables` tab lists all of the
placeholders in the template, as well as the variables that should be populated. For example,
messages can contain placeholders for a recipient's name, purchased products, or sales order number.
.. ADD BUTTONS TAB DETAILS HERE ..
...
.. note::
   Buttons can also be added on the `Meta business console
   <https://business.facebook.com/wa/manage/home>`_. To see Meta's WhatsApp template dashboard, go
...
.. tip::
   To see the status, go to `Meta's WhatsApp template dashboard
   <https://business.facebook.com/wa/manage/home>`_ and select :menuselection:`Account tools -->
   Message templates`.
...
name on Meta's business manager and in Odoo. This causes a duplicate validation error. Odoo will
display `Validation Error: The operation cannot be completed: Duplicate template is not allowed for
one Meta account.` To correct this issue, rename the duplicate template name on Odoo and :ref:`sync
the templates once again <productivity/whatsapp/sync>`.
...
Error validating access token: Session has expired`. To correct this issue, :ref:`add a permanent
token <productivity/whatsapp/token>`.
