...
**Odoo Dashboards** allows you to consult and interact with, customize, and build interactive
...
On the main Dashboards page, the left panel lists all :ref:`standard dashboards
<dashboards/standard>` and :doc:`custom-built dashboards
...
Clicking on a dashboard name opens that dashboard in the main part of the page.
...
.. tip::
   ...
   - Mark a dashboard as a favorite for by clicking the :icon:`fa-star` :guilabel:`(Toggle
     favorite)` icon at the top right of the page. Favorited dashboards are grouped in a
     :guilabel:`Favorites` section at the top of the left panel for easy access.
...
If :doc:`global filters <spreadsheet/global_filters>` have been created for a dashboard, a search
bar at the top of the page allows you to filter the entire dashboard for the selected values.
...
.. tip::
   ...
   - It is also possible to :ref:`searching for values <search/values>` by typing directly in the
...
#. For the relevant filter, select an operator from the first dropdown, then select the desired
...
Depending on the dashboard and the table in question, it may be possible to sort data by column by
clicking the :icon:`fa-sort` :guilabel:`(sort)` icon beside the column heading. A
:icon:`fa-caret-down` :guilabel:`(caret down)` or :icon:`fa-caret-up` :guilabel:`(caret up)` icon
indicates that the column has been sorted in descending or ascending order, respectively.

.. tip::
   When conditional formatting rules have been defined for a specific data range of a table, and
   the table is sorted by a different column in the dashboard view, the formatting moves with the
   data to the cell's new location.
   ...
   .. example::
      ...
      bars. When resorted by number of leads in descending order, the data bars still reflect the
...
While a dashboard provides a user-friendly summary of key data, drilling down to the underlying data
...
To see dashboard charts in full screen, hover over a chart and click the :icon:`fa-expand` icon.

It is also possible to copy or download a dashboard chart by hovering over the chart and clicking
the :icon:`fa-ellipsis-v` (vertical ellipsis) icon, then clicking :icon:`os-copy-as-image`
:guilabel:`Copy as image` or :icon:`fa-download` :guilabel:`Download` as relevant.

.. tip::
   Once a time-series chart, i.e., a chart showing data over time, is expanded to full screen, it is
   possible to :ref:`navigate the chart <dashboards/use-dashboards/navigate-charts>` by zooming and
   scrolling.

.. _dashboards/use-dashboards/granularity-charts:

Adjust granularity of time-series charts
----------------------------------------
...
.. _dashboards/use-dashboards/navigate-charts:

Zoom and scroll time-series charts
----------------------------------
...
- zoom in on a portion of the entire time period presented;
- when you have zoomed in, to scroll through the chart, keeping the same level of zoom.

To zoom in, drag the ends of the slider to select the portion of the time period you want to see,
e.g., to show five days or four weeks. To scroll through the chart while maintaining the level of
zoom, drag the slider to the left or right. Resize the slider to return to the initial view.

.. example::
   In this example from the Warehouse Daily Operations dashboard, the chart shows transfers yet to
   be assigned to a responsible user, by day. The zoom of the chart has been adjusted to show data
   for five days, and the current position of the slider allows data from the last five days to be
   shown.
...
<dashboards/access-and-sharing>` can consult and interact with them. Dashboard-specific filters,
available via the search bar, allow the entire dashboard to be filtered for selected values.
