...
.. tip::
   ...
   between spreadsheet tabs. To close the properties pane, click the :icon:`fa-times` (close) icon
...
.. note::
   ...
   inserted does not delete the underlying data source; the data source of an inserted list or pivot
   table can only be deleted via the data source's properties.
...
#. With the list view (`oi-view-list`) selected, click the :icon:`fa-cog` :guilabel:`(Actions)` icon
...
#. Click :guilabel:`Blank spreadsheet` or select in which existing spreadsheet the list should be
   ...
   .. note::
      ...
      Drive` personal workspace or, if file centralization has been enabled for spreadsheets, in the
      :guilabel:`Spreadsheet` workspace.

#. Click :guilabel:`Confirm`.

The list is inserted in a new sheet in the spreadsheet. The sheet tab in the bottom bar shows the
...
When a list is inserted in a spreadsheet, the following :doc:`functions <functions>` are used to
...
.. tip::
   ...
   the menu bar. The example below shows the functions used to retrieve list values.
...
- :guilabel:`Sorting`: how the data is sorted, if applicable. To add a sorting rule, click
  ...
  :guilabel:`Descending`. Delete a sorting rule by clicking the :icon:`fa-times` (delete) icon.

To :ref:`duplicate <spreadsheet/insert/list-duplicate>` or :ref:`delete
<spreadsheet/insert/list-delete>` a list's data source, click the :icon:`fa-cog` (gear) icon then
:icon:`fa-clone` :guilabel:`Duplicate` or :icon:`fa-trash` :guilabel:`Delete` as relevant.
...
- Select the last row of the table then hover over the blue square until the plus icon appears.
  ...
  :ref:`spreadsheet formula <spreadsheet/insert/list-functions>` is extended to the new cells. If
...
#. Select the appropriate field name and press `Enter`. The field's label appears in the header.
   ...
   .. tip::
      ...
      <developer-mode>` and check the field name by hovering over the question mark beside a field's
      label.
...
(gear) icon then :icon:`fa-clone` :guilabel:`Duplicate`.
...
#. Add a new sheet by clicking the :icon:`fa-plus` (add sheet) icon at the bottom left of the
   screen.
...
- From the :ref:`properties pane <spreadsheet/insert/list-properties>` of the relevant list, click
  the :icon:`fa-cog` (gear) icon then :icon:`fa-trash` :guilabel:`Delete`. This deletes the data
  source of the list from the spreadsheet.
...
#. With the pivot view (:icon:`oi-view-pivot`) selected, click :guilabel:`Insert in Spreadsheet`.
...
The pivot table is inserted in a new sheet in the spreadsheet. The sheet tab in the bottom bar
...
- `measure_name`: the name of the quantatitive data being aggregated.
...
.. tip::
   To see the formulas of spreadsheet cells, click :menuselection:`View --> Show --> Formulas` on
   the menu bar. The example below show sthe functions for a static pivot table.
...
- :guilabel:`Columns` and :guilabel:`Rows`: dimensions you are using to categorize or group data
  from the model.
- :guilabel:`Measures`: the quantative date you are measuring, or analyzing, based on the dimensions
  you have chosen.
...
(gear) icon then :icon:`fa-copy` :guilabel:`Duplicate` or :icon:`fa-trash` :guilabel:`Delete`.
...
#. With the :ref:`pivot table properties <spreadsheet/insert/pivot-table-properties>` open, click
   the :icon:`fa-cog` (gear) icon then :icon:`fa-clone` :guilabel:`Duplicate`.

   The duplicated pivot table is automatically inserted in a new sheet in the spreadsheet with the
...
- From the :ref:`properties pane <spreadsheet/insert/pivot-table-properties>` of the relevant pivot
  table, click the :icon:`fa-cog` (gear) icon then :icon:`fa-trash` :guilabel:`Delete`. This deletes
...
#. With the graph view (:icon:`fa-area-chart`) selected, click :guilabel:`Insert in Spreadsheet`.
...
prefaced by the :icon:`fa-bar-chart` (chart) icon. Alternatively, hover over the chart then
click the :icon:`fa-bars` (menu) icon and click :icon:`fa-pencil-square-o` :guilabel:`Edit`.
...
- :guilabel:`Chart type`: the type of chart. By default, this indicates the type of chart you
  ...
  After a chart has been inserted, more chart types are available; click the drop-down menu to
...
.. note::
   ...
   - It is also possible to insert a clickable link to a specific view of a model in a spreadsheet
     starting from the model view itself. However, as this method inserts each new link in a new
     sheet, it is more efficient to create links to specific views starting from the spreadsheet.

You can :ref:`insert a clickable link from any chart <spreadsheet/insert/clickable-links-chart>` to
a specific view of a model.
...
#. Click :menuselection:`Insert -->` :icon:`fa-link` :menuselection:`Link` from the menu bar or
   ...
   - Click the :icon:`fa-bars` (menu) icon, then :guilabel:`Link an Odoo menu`. Select the relevant
     menu item from the list or click :guilabel:`Search more` to choose from a list of all menu
     items. Click :guilabel:`Confirm`.
   - Click the :icon:`fa-bars` (menu) icon, then :guilabel:`Link sheet`, then choose the relevant
     sheet from the current spreadsheet.
...
#. Hover over the top right of the chart's box then click the :icon:`fa-bars` (menu) icon, then
...
#. At the bottom of the :icon:`fa-sliders` :guilabel:`Configuration` tab of the chart properties
   ...
   Hover over the top right of the chart's box to see that a new :icon:`fa-external-link` (external
   link) icon has been added.

.. tip::
   Clicking on a data point in a chart takes you to the relevant database view. In the example
   below, clicking on :guilabel:`Jessica Childs` takes you to the list view of all sales by this
...
accounting data from your database and insert it in a spreadsheet.
