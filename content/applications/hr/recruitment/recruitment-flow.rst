...
When the applicant's card is created, Odoo automatically populates the :guilabel:`Candidate`,
:guilabel:`Email`, and :guilabel:`Phone` number, on the applicant's card. This information is
required when applying for a job position, by default.
...
application, it populates the chatter at the bottom of the applicant's record, as a note from
:guilabel:`OdooBot`, with a headline of `Other Information`.
...
<recruitment/settings>` of the **Recruitment** app, the resumé appears on the applicant's card, on
the right-hand side.
