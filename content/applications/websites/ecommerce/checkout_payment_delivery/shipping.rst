...
.. tip::
   ...
   - Use the :guilabel:`Availability` tab to define conditions for the
     :ref:`delivery method <inventory/shipping_receiving/availability>` based on the order’s content
     or destination.
...
#. Scroll to the :guilabel:`Delivery` section;

#. Enable :guilabel:`Click & Collect`;
...
:guilabel:`Stores` tab, click :guilabel:`Add a line` and select the :guilabel:`warehouse(s)` where
...
:guilabel:`Publish`, making the delivery method available to customers.
...
.. note::
   ...
   - When the product is in stock, a location selector is displayed on :doc:`product <../products>`
     and :doc:`checkout <checkout>` pages.
