...
Knowledge articles can be created from scratch or from a preconfigured template. When one article
is created beneath another, the original article becomes the **parent article**. The article created
within it is referred to as a **nested** or **child article**, signifying its subordinate position.
This structure helps organize content by establishing clear relationships between related articles.
...
viewing of various types of organized content and data.

When article items are part of a parent article, they can also display :ref:`properties
<knowledge/articles_editing/properties>`. The latter are shared data fields from the parent,
ensuring consistent information across all related items and articles.
...
Higher-level article influence lower-level articles (**parent** and **nested** articles): Nested
articles inherit their parent's :ref:`access rights <knowledge/articles_editing/rights>`, and
   ...
   .. list-table::
      ...
      * - :guilabel:`Members`
        - The article is only visible in the sidebar tree to :ref:`invited users
          <knowledge/articles_editing/invite>`, while other users can find it using the hidden article
...
Properties are custom fields for storing and managing information that any user with
`Can edit` :ref:`access rights <knowledge/articles_editing/rights>` can add to :ref:`nested articles
...
.. note::
  ...
  - To remove a property, hover over its name, click the :icon:`fa-pencil` (:guilabel:`pencil`)
    ...
    deleting all properties removes the property sidebar
    panel.
...
.. tip::
  ...
  - Tick :guilabel:`Display in Cards` to show the properties in an :ref:`article item's view
    <knowledge/articles_editing/items>`, which is visible from a parent article.
