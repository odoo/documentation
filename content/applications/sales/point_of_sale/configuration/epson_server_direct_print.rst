...
The Epson Server Direct Print functionality allows for receipt printing from Point of Sale systems. Once
enabled, the POS register sends tickets directly to any Epson printer model supporting Server Direct Print.

.. note::
   The following Epson printers are :doc:`compatible
   </applications/general/iot/devices/printer>` with this feature.
   ...
   They don't require connecting to an :doc:`IoT system
   </applications/general/iot/connect>`.
   Our recommended model for receipt printing is :guilabel:`TM-m30III`
...
.. note::
   ...
   - Ensure the printer's :guilabel:`Device ID` is set to `local_printer` in the :guilabel:`Device Admin` section.
