...
.. tip::
   Account codes must be unique per chart of accounts, so ensure that any newly created or imported
   accounts do not share the same code as an existing account.
...
- For vendor bills and refunds, go to :menuselection:`Accounting --> Vendors --> Bills`, click the
  :icon:`fa-cog` (:guilabel:`Actions`) icon, and click :icon:`fa-download` :guilabel:`Import records`.
...
(:guilabel:`Actions`) icon, and click :icon:`fa-download` :guilabel:`Import records`. Click
