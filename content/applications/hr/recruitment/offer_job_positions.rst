...
a contract <recruitment/offer_job_positions/contract-proposal>`. Once the contract is :ref:`fully
signed <recruitment/offer_job_positions/contract-signed>`, the :ref:`new employee record is created
<recruitment/offer_job_positions/new-employee>`.
...
.. _recruitment/offer_job_positions/contract-proposal:
...
applicant card. Click the :guilabel:`Contract Proposal` stage in the status bar at the top of the
applicant's record.
...
Most fields are pre-populated with information from the job position. Update or add any relevant
...
- :guilabel:`Contract Template`: The template currently being used to populate the *Offer for
  ...
  .. note::
     To modify the template, hover over the current template name, click the :icon:`fa-arrow-right`
     :guilabel:`(Internal link)` icon that appears to the right of the field, and make any desired
     changes.
...
- :guilabel:`Salary Simulation Preview`: The various costs for the contract appear in this box,
  ...
  :guilabel:`Company Cost`. These numbers are estimates and should be used for reference only.
...
.. _recruitment/offer_job_positions/configure-package:
...
transferred to the employee's record once created.
...
click the :guilabel:`Validate & Send Completed Document` button, and the contract is signed.
...
.. image:: offer_job_positions/hired.png
   :alt: Hired banner in the corner of applicant card.
