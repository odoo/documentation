...
country-specific e-invoicing format option (such as :guilabel:`BIS Billing 3.0` for Belgium) to
...
affecting real data, while :ref:`live <accounting/e-invoicing/peppol-registration-production>` mode
...
The following fields in the :guilabel:`PEPPOL Electronic Document Invoicing` section are generally
...
.. tip::
   For **advanced users only**, running tests on Peppol's test network is possible using the test
   mode. The server allows users to register on Peppol and to send/receive test invoices with other
   participants. To do so, follow these steps:

   #. To deregister from the demo mode, go back to the :guilabel:`PEPPOL Electronic Document
      Invoicing` section and click :guilabel:`Deregister from Peppol`.
   #. Enable :ref:`developer mode <developer-mode>`, open the Settings app, go
      to :menuselection:`Technical --> System Parameters`, and search for
      :guilabel:`account_peppol.edi.mode`.
   #. Open the parameter and change the :guilabel:`Value` to `test`.
   #. Go back to the :guilabel:`PEPPOL Electronic Document Invoicing` section; and click
      :guilabel:`Validate registration (Test)`.
   #. Update the :guilabel:`Mobile Number` and click :guilabel:`Verify mobile number` to verify it.
...
#. To deregister from the demo mode, go back to the :guilabel:`PEPPOL Electronic Document Invoicing`
   section and click :guilabel:`Deregister from Peppol`.
#. Enable :ref:`developer mode <developer-mode>`, open the Settings app, go to
   :menuselection:`Technical --> System Parameters`, and search for
   :guilabel:`account_peppol.edi.mode`.
#. Open the parameter and change the :guilabel:`Value` to `prod`.
#. Go back to the :guilabel:`Peppol Electronic Document Invoicing` section, and fill in the
...
#. The registration should be automatically activated within a day.
   ...
   .. tip::
      ...
      :ref:`developer mode <developer-mode>`, open the Settings app, go to :menuselection:`Technical
      --> Scheduled actions`, and search for :guilabel:`Peppol: update participant status`. Open the
      scheduled action, then click :guilabel:`Run Manually`.
...
.. tip::
   To manually trigger the scheduled action used to check the invoices' status, click
   :guilabel:`Fetch Peppol`.
