...
Domain filters allow you to determine on which records an automation rule acts, or does not act.
...
- a :guilabel:`Delay`: Specify the number of minutes, hours, days, or months. To have an action
  ...
  .. note::
     ...
     hours. When you set a :guilabel:`Delay` that is less than the equivalent of 2400 minutes, or 40
     hours, the system recalculates the frequency of this check to ensure the automation rule is
     triggered close to its scheduled time.
     ...
     <developer-mode>`, go to :menuselection:`Settings app --> Technical --> Scheduled
     Actions` to see all scheduled actions for your database.
