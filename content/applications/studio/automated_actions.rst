...
These actions are used to subscribe/unsubscribe existing contacts to/from the record. With
:guilabel:`Add Followers` or :guilabel:`Remove Followers` selected as the
:guilabel:`Type` of action, as relevant, select a :guilabel:`Followers Type`:

- :guilabel:`Specific Followers` allows the same, specific contact(s) to be added/removed. To add
  followers, in :guilabel:`Followers to add`, select the relevant contact(s) from the dropdown. To
  remove followers, click the :icon:`fa-times` :guilabel:`(cross)` beside a contact's name.

- :guilabel:`Dynamic Followers` allows contacts linked to the record to be added/removed
  dynamically. To add/remove followers dynamically, in :guilabel:`Followers Field`, enter the
  technical name of the relevant field.

.. tip::
   To find a field's technical name, with :ref:`developer mode <developer-mode>` activated, use one
   of the following methods:

   - In a record of the relevant model, hover over the tooltip beside the field name. The field's
     technical name is found beside :guilabel:`Field`.
   - Go to :menuselection:`Settings --> Technical --> Models` and open the relevant model. The
     technical name of the model's fields is found in the :guilabel:`Field Name` column of the
     :guilabel:`Field` tab.

.. example::
   To keep customers informed of progress on a project, this automated action adds the relevant
   customer as a follower when a project task is set to :guilabel:`In progress`. The technical name
   of the customer field, i.e., `partner_id` is used.
