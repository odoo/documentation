...
#. Setup your tour by registering it. All tours must be added to web_tour.tours registry.

   .. code-block:: javascript

      registry.category("web_tour.tours").add('name_of_the_tour', {
...
- **trigger**: Required, Selector/element to ``run`` an action on. The tour will
  ...
  action *on it*.
  The trigger must be a CSS selector (string). The tour engine uses the Hoot library
  (made by odoo) to search the DOM for the element targeted by the selector.
  We can therefore use :ref:`Hoot pseudo-selectors <hoot/custom-dom-selectors>` in the trigger.
...
- **run**: Optional, Action to perform on the *trigger* element. If no ``run``,
  ...
  - The name of one of the action helpers, which will be run on the
    trigger element. Note that following actions are based on
    :ref:`Hoot interaction helpers <hoot/interaction_helpers>`:
