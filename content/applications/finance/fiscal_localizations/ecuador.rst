...
- :guilabel:`Identification Number`: For a company, enter the `RUC`. For individuals, enter the
...
- :guilabel:`Email`: Enter the company or individual's email. This email will be used when sending
  electronic documents, such as invoices.
...
:menuselection:`Accounting app --> Configuration --> Settings`, and scroll to the
:guilabel:`Ecuadorian Localization` section.
...
- :guilabel:`Use production servers`: Tick this checkbox if your company is going to do electronic
  documents in the production environment. To use the testing environment for electronic documents,
...
To configure a VAT withholding, go to :menuselection:`Accounting app --> Configuration -->
Taxpayer Type SRI`.
...
.. tip::
   In case the :guilabel:`Taxpayer Type` is `RIMPE`, also configure the :guilabel:`Profit
...
To configure your printer points, navigate to :menuselection:`Accounting app --> Configuration -->
Journals`.
...
- :guilabel:`Type`: This field refers to the type of journal; select :guilabel:`Sales`.

Once the :guilabel:`Type` is selected, complete the following fields that appear:

- :guilabel:`Use Documents?`: Tick if legal invoicing (invoices, debit/credit notes) is used, this
  is the typical configuration. If not, set means that will be used to register accounting entries
  not related to invoicing legal documents. For example: receipts, tax payments, register journal
  entries.
...
- :guilabel:`Dedicated Credit Note Sequence`: Check this checkbox if *credit notes* should be
  generated from this printer point (i.e. the journal).
- :guilabel:`Dedicated Debit Note Sequence`: Check this checkbox if *debit notes* should be
  generated from this printer point (i.e. the journal).
...
A *withholding journal* must be defined by going to :menuselection:`Accounting app --> Configuration
--> Journals`. For each withholding journal, click :guilabel:`New`, and enter the following
information on the journal form:

- :guilabel:`Journal Name`: This field should be entered in this format:
  `[Emission Entity]-[Emission Point] [Document Type]`. For example: `001-001 Withholding`.
- :guilabel:`Type`: This refers to the type of journal. Select :guilabel:`Miscellaneous`.
...
- :guilabel:`Short Code`: This is the unique code for the sequence of accounting entries. Enter a
  unique 5-digit code. For example: `WT001`.
...
A specific journal must be created for *purchase liquidations*. Go to :menuselection:`Accounting app
--> Configuration --> Journals`. Click :guilabel:`New`, and enter the following information on the
journal form:

- :guilabel:`Journal Name`: This field should be entered in this format:
  `[Emission Entity]-[Emission Point] [Document Type]`. For example:
  `001-001 Purchase Liquidations`.
- :guilabel:`Type`: This refers to the type of journal. Select :guilabel:`Purchase`.
...
- :guilabel:`Emission address`: Enter the address of the establishment in this field.
- :guilabel:`Short Code`: This is the unique code for the sequence of accounting entries. Enter a
  unique 5-digit code. For example: `PT001`.
...
.. seealso::
   :doc:`Manage chart of accounts <../accounting/get_started/chart_of_accounts>`
...
Go to :menuselection:`Accounting app --> Vendors --> Products`. On the :guilabel:`General
Information` tab, both :guilabel:`Purchase Taxes` and :guilabel:`Profit Withhold` can be specified.

Review taxes
~~~~~~~~~~~~

As part of the localization module, taxes are automatically created with its configuration and
related financial accounts. When creating an additional tax, base it on the configuration of the
existing taxes.

To manage taxes, navigate to :menuselection:`Accounting app --> Configuration --> Taxes`. Depending
on the tax type, the following options may be required for additional configuration:

- :guilabel:`Tax Name`: This field follows a specific format depending on the type of tax:
...
- :guilabel:`Tax Support`: This field should only be configured in the IVA tax. This option is
  useful when registering purchase withholdings.
- :guilabel:`Code ATS`: This field should only be configured for income tax withholding codes. It is
  important to use when registering a withholding.
- :guilabel:`Tax Grids`: This field is for configuring the codes of a 104 form if it is a IVA tax,
  and the codse of a 103 form if it is an income tax withholding codes.
...
Document types can be accessed or configured by going to :menuselection:`Accounting app -->
...
The information required for the document types is included by default, so you do not need to change
anything there.

Workflows
=========

Once a company's database is configured, documents can be registered so that workflows can be
completed across Odoo's applications, such as **Accounting**, **Inventory**, and **Sales**.
...
(posted) invoice to register a credit note.

On the invoice, there is a smart button named :guilabel:`Credit note`; click on this button to be
directed to the :guilabel:`Create credit note` form, then complete the following information:
...
.. seealso::
   :doc:`Manage customer credit notes <../accounting/customer_invoices/credit_notes>`
...
.. seealso::
   :ref:`Issue a customer debit note <accounting/issue-debit-note>`
...
the invoice, click the :guilabel:`Add Withhold` button to be directed to the :guilabel:`Customer
withholding` form, and complete the following information:
...
To create a new journal, go to :menuselection:`Accounting app --> Configuration --> Journals`, and
...
.. seealso::
   :doc:`Create vendor bills <../accounting/vendor_bills>`
...
It is necessary to have an invoice in a validated state before registering a :guilabel:`Purchase
withholding`. On the invoice, click the :guilabel:`Add Withhold` button. From there, complete the
following fields on the :guilabel:`Withholding` form:
...
- :guilabel:`Legal Entity`: reimbursement for incurred expenses, such as representation expenses
  (e.g. hiring a lawyer)
- :ref:`Vendor Bills <l10n_ec/vendor-bills>`
...
To enable expense reimbursement for individuals, make sure a :ref:`purchase liquidation journal
<l10n_ec/purchase-liquidation>` has been created.
...
- :guilabel:`Reimbursement Lines` tab: Add the expenses line by line, and provide the following
  details for each expense:
...
:ref:`vendor bills journal <l10n_ec/vendor-bills>` has been created.
...
Next, to create the reimbursement, :ref:`create a vendor bill <l10n_ec/vendor-bills>` using the
...
.. seealso::
   `eCommerce documentation <../../websites/ecommerce>`
...
To generate an invoice after the checkout process, navigate to :menuselection:`Website app -->
...
:guilabel:`View other providers` button under the :guilabel:`Activate Payments` heading.

From here, each payment provider can be configured by selecting a provider record. Refer to the
:doc:`payment provider <../payment_providers>` documentation for more information.

.. seealso::
   :doc:`Payment provider documentation <../payment_providers>`

Payment methods
^^^^^^^^^^^^^^^

To activate one or more payment methods for a payment provider, click :icon:`oi-arrow-right`
...
To get these reports, go to :menuselection:`Accounting app --> Reporting --> Statements Reports -->
Tax Report` and then filter by `Tax Report 103` or `Tax Report 104`.
