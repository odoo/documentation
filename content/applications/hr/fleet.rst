...
The :guilabel:`End Date Contract Alert` field determines how many days before the end of a vehicle
contract an alert should be sent. The responsible people receive an email informing them a vehicle
...
.. note::
   ...
   :guilabel:`Contracts` smart button at the top of the page. The contracts associated with this
...
The :guilabel:`New Vehicle Request` field determines when a warning appears in the salary configurator.
An employee filling out the salary configurator form (after being offered a position), will *not* be able to request a new car if 
the number of existing cars is greater than the number specified in the :guilabel:`New Vehicle Request`
field. Enter the specific number limit for existing available cars in this field.
...
Odoo's **Fleet** app comes preconfigured with sixty-six commonly used car and bicycle manufacturers in the
...
configured in the database. Remove the defaut filter to view *all* manufacturers.

The manufacturers appear in an alphabetical list. Each manufacturer's card lists how many specific
:doc:`models <fleet/models>` are configured for each particular manufacturer.
...
To add a new manufacturer to the database, click :guilabel:`New` in the upper-left corner, and a manufacturers
form will load. Only two pieces of information are needed, the :guilabel:`Name` of the manufacturer, and the logo.
Type the name of the manufacturer in the :guilabel:`Name` field, and select an image to upload for the logo.
