...
implemented by default for every test that spawns an **Owl environment** [#]_.
...
.. [#] A mock server is needed as soon as an environment is spawned because some **services**
    do send server requests as soon as they start.

Overview
========

A mock server is actually quite simple in conception in itself: it is an **object**
...
are spawned by **services** as soon as an **Odoo environment** is spawned.
...
- a **class** extending the `models.Model` class;
    ...
    - other `public class fields <https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Classes/Public_class_fields>`_
...
is expected to be a **route** callback, receiving a `Request <https://developer.mozilla.org/en-US/docs/Web/API/Request>`_
object:
...
of a mock `Response <https://developer.mozilla.org/en-US/docs/Web/API/Response>`_ object.
...
In such cases, an *optional* dictionary can be passed as a **3rd argument** to
...
- as `public class fields <https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Classes/Public_class_fields>`_;
