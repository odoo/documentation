...
- a :guilabel:`Delay`: Specify the number of :guilabel:`Minutes`, :guilabel:`Hours`,
  ...
  .. note::
     ...
     In the `Automation Rules: check and execute` scheduled action that opens, update the value
     of the :guilabel:`Execute Every` field, if desired. Clicking :guilabel:`Run Manually` triggers
     the scheduled action to run immediately. To return to the automation rule setup, click the
...
- :guilabel:`On create`: when a record is saved for the first time, e.g., .
...
define the action(s) to be executed.
