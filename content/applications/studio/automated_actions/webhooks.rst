...
.. note::
   ...
   it is also possilbe to create an automated action that :ref:`sends data to an external webhook
...
#. If the system sending the webhook is not Odoo, adjust the :guilabel:`Target Record` code to look
   ...
   If the webhook is used to create a record in the Odoo database, use `model.browse(i)` or
...
.. tip::
   - Webhooks can be also be created via the :guilabel:`Automations` menu in **Studio** by
     selecting the trigger :guilabel:`On webhook`.
...
response code, indicated whether or not the webhook is functioning correctly.
...
#. In your Odoo database, choose a sales order to test the webhook on. In the pasted code, replace
   ...
   .. tip::
      ...
      a sample sales order and the webhook that was created.
