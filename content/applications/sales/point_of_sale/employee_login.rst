...
:icon:`fa-ellipsis-v` (:guilabel:`vertical ellipsis`) button on the top-right corner of a POS card
on the main POS dashboard. Then,

#. Activate the :guilabel:`Multi Employees per Session` feature.
#. Add the employees who are allowed **basic POS functionalities** in the :guilabel:`Basic rights`
#. Add the employees who are allowed **extended POS functionalities** in the :guilabel:`Advanced
   rights` field.
...
POS session <pos/session-start>` and access the POS interface. They can either :ref:`scan their
...
To enable employee badge-based login, each employee must have a unique badge ID assigned to
