...
Adding a date range requires two fields: the :guilabel:`Date Range` field itelf and another field
that is selected as the start date *or* end date. This underlying field can be an existing
:ref:`Date field <studio/fields/simple-fields-date>` or :guilabel:`Date & Time` field, or one
...
#. Identify an existing :guilabel:`Date` or :guilabel:`Date & Time` field that can be used as the
   ...
   .. tip::
      ...
      made invisble by enabling :guilabel:`Invisible` or removed from the view by clicking
...
- :guilabel:`Latest accepted date`: Enter the latest date that can be selected in the date
   ...
   date, enter `today`. On the date selector, dates after than the latest accepted date are grayed
   out.
