...
Finally, :ref:`create an invoice from the sales order <accounting/invoice/invoice-from-sales>`, and
confirm it. The invoice lines are created according to the distribution of the delivery order, in
cases where more than one customs number were selected, Odoo will split the invoice lines by
quantity and customs numbers.
