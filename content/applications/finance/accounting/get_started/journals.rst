...
- :guilabel:`Short Code`: Each journal must have a unique code. The short code is used as the prefix
  for all journal entries belonging to this journal.
...
- :guilabel:`Email Alias`: Set an email address to create journal entries by digitizing PDFs sent
  via email. This is most commonly used to create :ref:`customer invoices and vendor bills
  <invoice-digitization/email-alias>`.
...
.. note::
   ...
   - If an :doc:`alias domain <../../../websites/website/configuration/domain_names>` has not yet
     been configured, a link to :icon:`fa-arrow-right` :guilabel:`Configure Alias Domain` is
     displayed instead of the :guilabel:`Email Alias` field.
...
To configure :doc:`payments <../payments>` to create journal entries, set outstanding accounts for
the journal's payment methods. This can be done for any journal with the :ref:`type
...
- :guilabel:`Account Number`: The bank account's number is used when registering payments and is
  ...
  :ref:`NACHA <l10n_us/nacha>`. To edit the bank account details, click on the external link arrow
  next to the :guilabel:`Account Number` and update the account information accordingly.
- :guilabel:`Bank`: The bank name is used when registering payments and is required for generating
  outgoing payment files, such as :doc:`SEPA <../payments/pay_sepa>` or :ref:`NACHA
  <l10n_us/nacha>`. To edit the bank account details, click on the external link arrow next to the
  :guilabel:`Bank` name and update the bank information accordingly.
- :guilabel:`Bank Feeds`: Define the method of creating bank :doc:`transactions
  <../bank/transactions>`, whether manual or via :doc:`Online Synchronization
...
.. image:: journals/journals-bank-fields.png
   :alt: The fields on bank journals
