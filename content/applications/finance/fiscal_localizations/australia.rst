...
.. _australia/payroll/work-entry-source:
   ...
   c. (if using the Attendances or Planning app) Select the :guilabel:`Work Entry Source` to define
      ...
      .. important::
         Timesheets do not impact work entries in Odoo. If you need to import your timesheets in
         Odoo, import them by going to :menuselection:`Payroll --> Work Entries --> Work Entries`
         instead.
...
:menuselection:`Payroll -->vReporting --> Super Contributions`.
...
Resetting payslips does not create new payslips or a new payrun, but instead:
...
- A new STP submission is created to replace the former one. For traceability purposes, the former
  STP submission is not be deleted but marked as replaced.
...
When ready, submit the pay run to the ATO once again. If the data sent to the ATO is still
incorrect, note that full file replacements can only be done once every 24 hours. If the same pay
run needs to be replaced a second time, make sure not to validate any new payslip or payslip batch
until the full file replacement with accurate data has been submitted.

Zero out YTD values
*******************

In case of a mid-year change of several key identifiers, YTD values need to be zeroed out, then
...
#. Before updating any key identifiers, create a new STP submission by going to
   ...
   - (if only individual employees should be zeroed out) Click :guilabel:`Add a line` to specify
     which employees.
...
- :guilabel:`Unused Leave Type`: choose between :guilabel:`Annual`, :guilabel:`Long Service`, or
  ...
  for this time off type will not show up as an entitlement at time of termination. Unused leaves of
  the type :guilabel:`Annual` will include leave loading, if the employee is eligible for it.
...
- Allowances subject to a varied rate of withholding (such as *cents per kilometre* allowance and
  *travel allowances*)
...
alternative payroll solution. The Employment Hero module synchronises payslip accounting entries
(e.g., expenses, social charges, liabilities, taxes) automatically from Employment Hero to Odoo.
Payroll administration is still done in Employment Hero. We only record the journal entries in Odoo.
...
he same record in Employment Hero and Odoo.
...
to :menuselection:`Accounting --> Configuration --> Settings` and, in the :guilabel:`Employment
Hero`, click :guilabel:`Fetch Payruns Manually`.
...
ero business (same name and same code) in Odoo. You also need to choose the correct account types in
